> **ARQUIVADO na v0.270.** *De onde saiu:* o capítulo 35 do Manual da Guilda, `sistema/05-material/livro/manual/35-caminhos-e-trilhas.md`, inteiro, como estava na v0.269.
>
> **O que o substituiu:** o mesmo capítulo reescrito pela coleção v0.4 dos Caminhos, que mora em `caminhos/` na raiz — Bastião, Vanguarda, Emanador e Guia, com as doze Trilhas novas e as três rotas do Batedor.
>
> **Por que morreu:** o Mizuki refez as habilidades base e as Trilhas dos quatro Caminhos fora desta pasta, e pediu a coleção no livro, *"exatamente pela coleção v0.4"*. **A base dos Caminhos não mudou** — vida, PE, atributos, perícias e treino de arma continuam os daqui. **E o Evocador saiu da edição jogável**, com as três Trilhas dele (`Servo`, `Matilha` e `Coro`), até o subsistema de Invocações fechar.
>
> **O que sobreviveu:** a abertura do capítulo (como ler um Caminho, vida e energia, termos, treino de arma), a base de cada um dos quatro Caminhos, e cinco nomes de Trilha — `Muro`, `Punho`, `Estocada`, `Batedor` e `Executor` mantêm o nome, com entregas novas. **O preço de cada entrega daqui continua nos `DESENHO-*.md` da raiz, e não vale para as entregas novas**, que não foram medidas. A seção do Evocador também está em `invocacoes/museu/35-evocador.md`, junto do capítulo 60.
