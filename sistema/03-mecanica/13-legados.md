...
**⚠ A pergunta de formato que ficou aqui da v0.176 até a v0.179 FECHOU, e ela se respondeu contando o catálogo.** *Ela dizia que o `Desliga` apaga **o que chega em você** e o `Conhecimento Antigo` apaga **uma rolagem que você faria**, então ele seria mais perto do `Ajusta`.* **Duas coisas derrubam isso, e as duas são medida e não gosto.**

> **A frase que ela usava como régua já descrevia um catálogo mais estreito que o real, e antes desta entrada existir.** *Dos onze `Desliga` escritos, o `Ferro Velho` apaga cansaço — e o texto dele diz, com todas as letras, que "cansaço não é uma coisa que acontece com você" — e o `Sangue que Não é Sangue` apaga precisar comer, dormir e respirar.* **Nenhum dos dois é coisa que chega; os dois são coisa que o mundo cobra.** *A régua operante do §5 sempre foi "apaga o que ninguém comprou", e essa o `Conhecimento Antigo` passa limpo: ninguém compra o teste de reconhecer técnica.* **O §5 foi reescrito para parar de mentir sobre a própria lista.**
>
> **E `Ajusta` é impossível para ele, não indesejável.** *O `Ajusta` é definido por carregar relógio — a largura do gatilho escolhe o degrau na escada da peça 10 —, e este Legado é `sempre`.* **Movê-lo exigiria inventar um relógio, e isso mudaria o que ele faz:** hoje ele é automático dos dois lados, acerto na sua Era e falha nas modernas, e um relógio transforma isso em "algumas vezes por cena".
