...
- **⚠ Achado nesta passada, e ainda sem gate: nada impede uma Origem sem PE de escolher um Caminho construído em cima de PE.** *A peça 6 não cita Origem nenhuma, e esta peça não cita Caminho nenhum — a checagem simplesmente não existe entre as duas.* **O caso mais afiado é a Restrição Celestial pelo ramo da Maki e do Toji**: a Origem diz *"sem PE"*, com todas as letras — não é pouco PE, é zero, absoluto —, e o Emanador entrega **`PE por nível: 6`** como o número que cresce a cada nível. **É contradição de regra, não desperdício de escolha**: a ficha teria um atributo que a própria Origem proíbe de existir. *O caso mais brando é Sem Técnica: ela mantém PE, mas não tem Fundamento — e o Emanador inteiro (`Sangria`, `Resquício`, `Modelagem`, `Fonte`, `Acelerar`, `Vazão`, `Cheia`, `Transbordo`) é kit construído em cima de feitiço. Sem Fundamento, o Caminho não quebra a regra; ele só não tem em que gastar o que ganha.* **Nenhuma das duas travas existe hoje**, nem aqui nem na peça 6. *Registrado para decisão do Mizuki — se o gate é "Origem sem PE não pega Caminho de PE alto" ou algo mais estreito, e se ele veta ou só desaconselha.*
