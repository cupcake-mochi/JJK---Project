...
**A armadilha que continua real: confira `PULADA=0`.** Sem `python-docx` instalado (`pip install python-docx --break-system-packages`) os **cinco** que leem o manual pulam em vez de falhar, e saem com código 0. **Um deles pula tudo:** o `conferir-manual` sai no `except ImportError` antes da primeira checagem. **Desde a v0.101 os cinco dizem que pularam** — antes disso o `conferir-nomes` e o `conferir-pericias` imprimiam `TUDO OK` estando cegos. *E o `subir.sh` acusa junto: validador que pulou sai como `ok*` em amarelo, com o motivo do lado.* **Mesmo assim, leia a saída em vez do código de retorno.**
...
- **Arquivo fantasma: `ls` e `stat` mostram tamanho e inode certos, e `open()` devolve ENOENT enquanto os vizinhos da mesma pasta abrem.** *Um validador que falha com `FileNotFoundError` num arquivo que existe é isto, e não bug de código.* **Reescrever por cima NÃO reconcilia — a escrita nova também sai ENOENT.** *Medido na v0.101, contra o que este item dizia antes.* **O que reconcilia é escrever com OUTRO NOME e depois `mv` por cima**, porque o `mv` não abre o destino. Confira por md5 depois. **E o mesmo `mv` é o jeito de tirar arquivo da pasta**, já que apagar não dá: mande para `_to_delete/`.
