...
**v0.221.** Fases 0 a 3 fechadas; a Fase 4 (mecânica) está em andamento com **vinte e seis peças escritas e vinte e seis validadores passando**. O manual do Fundamento está na **v7.26**, e ele é um subsistema fechado — a técnica e o feitiço já funcionam. O `.pdf` **está na mesma versão do `.docx`** desde a v0.93 — ele deixou de ser exportado a mão. *⚠ Na v0.221 o `.docx` foi para a `v7.26` e o `.pdf` ficou na `v7.25`: a conversão pede `Georgia` e `Consolas`, e a leva rodou numa máquina que não tem as duas — assar fonte substituta dentro do artefato é o defeito que a própria leva foi consertar no livro.*
