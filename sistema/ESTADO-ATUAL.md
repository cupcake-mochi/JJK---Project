...
6. ~~**Se a curva de dano deve cruzar a coluna Rotina.**~~ **FECHADA na v0.72: sim, ela cruza, e por decisão.** *Ficou aberta desde a v0.27.* No nível 2 o conjurador está +38% e o físico +69% acima da Rotina; no nível 30 o físico terminava **2% abaixo**. **Com o orçamento de Caminho e Trilha dobrado, ele termina em `+14,6%` acima** — a curva cruza de propósito, e a camada de Caminho mais Trilha passa a valer de `14%` a `19%` do que o personagem faz, contra os `6%` a `10%` de antes.
