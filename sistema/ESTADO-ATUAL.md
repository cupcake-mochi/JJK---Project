...
Atualizado em 20/09/2026, na v0.261 (última peça fechada: **Bestiário**, a peça 26, na v0.198, com o `conferir-bestiario.py` e dez checagens; antes dela, **Sem Técnica, a peça 25, na v0.168**; antes dela, **Dano de alma e Integridade, na v0.145**). Este arquivo existe para retomar o trabalho — inclusive em conversa nova — sem recontextualizar tudo. Leia ele inteiro antes de mexer em qualquer coisa: ele tem a seção *"Onde estamos, e o que falta"* no fim, que é o ponto de retomada.

**Versão v0.261.** Fases 0 a 3 fechadas; Fase 4 (mecânica) em andamento, **vinte e seis peças escritas** e **vinte e seis validadores**.
...
11. **Ritual.** *Levantado pelo Mizuki em 19/09/2026: "A gente n fez sistema de sinais de ritual para feitiços e talz".*

    > ***DECIDIDO com ele em 20/09/2026, e ainda NÃO escrito. É a v0.260.***
    >
...
11. **Ritual.** *Levantado pelo Mizuki em 19/09/2026: "A gente n fez sistema de sinais de ritual para feitiços e talz".* **Hoje o `Selo` é o único gesto de conjuração:** *um só por técnica, sempre o mesmo, sem custo e sem bônus, e a Condição `Calado` impede o que precisa de voz.* **Não existe conjuração de ritual, com sinais próprios e tempo de minutos, para o feitiço especial:** *o mais lento que há é a Restrição `Atrasar` (a rodada inteira) e o `Carregar`.*
...
15. **A Melhoria de resistência a tipo — ADIADA na v0.258, e ela esperava um número que não existe.** *Levantada como candidata na v0.254, quando a curadoria de `33` proteções do D&D achou que resistência a dano é a maior categoria com `11` e o nosso catálogo de Melhorias não tinha nenhuma.* ***Decisão do Mizuki em 20/09/2026: "abandona por enquanto, e espera o playtest".*** **O que a v0.258 mediu, e fica pronto para quem voltar:**

    > **1 · A lacuna registrada na v0.254 estava errada pela metade, e é a lição nº 6.** *Aquela frase dizia "o catálogo só tem defesa e dividir dano", e ela lia só o catálogo de **Melhorias**.* **Resistência a um tipo já existe no Fundamento desde a v0.26: é a Passiva `Escama`, Classe Passiva `3`** — *"escolha um tipo de dano que a sua Regra justifique. Você tem resistência a ele: todo dano daquele tipo cai pela metade"*. *Então o `Corrói`, que fura resistência, sempre teve referente dos dois lados.* **A peça 13 já tinha registrado o mesmo erro com as mesmas palavras** — *"estava errado, e o erro era não ter procurado"* —, e ela nomeia a `Escama` como quem cobra por aquele degrau.
    >
    > **2 · O buraco real é mais estreito do que a candidata dizia.** *A `Escama` custa `3` espaços de feitiço conhecido e **só abre no nível 13**; no D&D existe resistência reativa desde o nível 1, pelo Absorver Elementos.* **O que falta é resistência CEDO, e comprável dentro de um feitiço.**
    >
    > **3 · A conta fecha.** *Moedas lidas dos donos: `1` ponto de feitiço vale `4,5` de dano causado (peça 19 §2.1) e `3,0` de dano evitado (a Forma `Apoio`: "cada ponto que sobra vira 3 de vida temporária", com o `1` pra `1` do `DESENHO-trilhas`).* **Medida contra o GOLPE do chefe, que é a régua defensiva da v0.201, e não contra a rodada:** *uma resistência que fica de pé, escopo = um grupo de dano, paga `72%` · `90%` · `99%` do preço `Pesada` nos níveis 10, 20 e 30 se o grupo for `Físicos`, e `63%` · `83%` · `92%` do preço `Leve` se for `Elementais`.* **Os dois grupos que fecham são exatamente os dois que o D&D usa** — *o Pele-Rocha pega cortante, perfurante e concussão e é de 4º círculo; a Proteção Contra Energia pega os elementais e é de 3º* —, e a conta chegou neles sem olhar o D&D.
    >
    > **4 · Um dos três escopos NÃO é calculável hoje, e é por isso que ela foi adiada.** *O Mizuki levantou três: um tipo fixo, um tipo escolhido na hora dentro de um grupo, e o grupo inteiro.* **O do meio só vale menos que o grupo inteiro se o inimigo usar mais de um tipo daquele grupo — e quantos tipos de dano um inimigo usa não existe em documento nenhum.** *A peça 26 nunca precisou desse número, então nunca o escreveu.* **Preçar o meio-termo exigiria inventá-lo.**
    >
    > **5 · E todo o preço fica pendurado num palpite que a própria peça 19 §4 declara:** *o peso dos três grupos — `Físicos 60%`, `Elementais 30%`, `Especiais 10%` — é **previsão**, e ela escreve que é "o número que decide quanto vale toda resistência do sistema, e o primeiro que a mesa vai corrigir".* **Não seria a primeira coisa na mesma corda** — *o `Alicerce` do `Muro` e a resistência de todo inimigo do Bestiário já dependem dele, e o multiplicador do §6.3 da peça 26 se refaz sozinho quando ele mudar* —, mas seria a terceira, e o playtest é quem destrava.
    >
    > **O nome já passou na triagem, se ela voltar: `Amortece`, LIVRE.** *Verbo na terceira pessoa, como `Corrói`, `Divide`, `Fura` e `Queima`.* **E `Aguenta` foi recusado por colisão de SENTIDO que a triagem não pega:** *`Aguentar` já é uma das duas escolhas de quem chega a `0` de vida.* *`Couro` saiu OCUPADO — é Legado do catálogo.*
    >
    > **Um número meu foi corrigido no caminho, e ele mudava a conclusão:** *eu tinha medido a `Guarda` contra o dano por RODADA e concluído que ela era dominada pela vida temporária da Forma `Apoio`.* **Refeita contra o golpe, ela paga `42%` a `52%` sozinha e `68%` a `90%` com a `Concentrada` — não há dominância, e o achado não existia.**
