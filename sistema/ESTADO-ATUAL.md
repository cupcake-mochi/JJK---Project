...
Atualizado em 25/09/2026, na v0.271 (última peça fechada: **Ritual**, a peça 27, na v0.262, com o `conferir-ritual.py` e sete checagens; antes dela, o **Bestiário**, a peça 26, na v0.198; antes dela, **Sem Técnica, a peça 25, na v0.168**; antes dela, **Dano de alma e Integridade, na v0.145**). Este arquivo existe para retomar o trabalho — inclusive em conversa nova — sem recontextualizar tudo. Leia ele inteiro antes de mexer em qualquer coisa: ele tem a seção *"Onde estamos, e o que falta"* no fim, que é o ponto de retomada.

**Versão v0.271.** Fases 0 a 3 fechadas; Fase 4 (mecânica) em andamento, **vinte e sete peças escritas** e **vinte e sete validadores**.

> ## A v0.271 fechou as decisões que a v0.270 deixou
>
> **A perícia fixa do Bastião passou a `Provocar`**, *como a v0.4 escreve, e a Kaori trocou `Intimidação` por ela.* **A frase dos Limites voltou, com o Socorrista como exceção declarada**, *e ninguém cura outra pessoa com Energia Reversa — a `Sutura` saiu e não tem sucessora.* **Três nomes da v0.4 mudaram no livro:** *`Eco` virou `Eco Amaldiçoado`, o `Impulso` do Catalisador virou `Impulso Energético` e a `Sobrecarga` virou `Sobre Carregar Energia`; o resto das colisões da triagem foi aprovado.* *Os renomes moram numa tabela da peça 6 §2, e a coleção em `caminhos/` fica como chegou. O porquê está na entrada da v0.271 do `CHANGELOG`.*

> ## A v0.270 pôs os Caminhos da coleção v0.4 no livro, e tirou o Evocador e as Invocações da edição jogável
>
> **Bastião, Vanguarda, Emanador e Guia, com as doze Trilhas e as três rotas do Batedor, entraram no capítulo 8 exatamente como a coleção v0.4 escreve**, *e ela passou a ser a dona do texto deles, em `caminhos/`. A base dos Caminhos não mudou; o degrau do nível 23 é novo nos quatro.* **O equilíbrio da v0.4 não foi medido** *— o Mizuki mandou implementar sem medir e pôr a medição na fila, e os `DESENHO-*.md` e a peça 17 ficaram como o registro com preço da coleção anterior.* **O Evocador e as Invocações saíram do livro até o subsistema fechar**, *com as fontes antigas em `invocacoes/museu/` e o desenvolvimento em `invocacoes/`. No livro de duas colunas, o capítulo 8 sai em coluna única. O porquê, as colisões de nome e os conflitos para ele decidir estão na entrada da v0.270 do `CHANGELOG`.*

> ## A v0.269 trocou a regra de queda do Domínio Simples, e a Cesta Oca passou ao teste do Carregar
>
> **Terceira volta do Mizuki sobre a conta do Simples:** *ele aguenta `3` rodadas de Expansão, `4` se a Essência de quem segura for maior que a do dono e `2` se for menor; cada Acerto que ele segura gasta uma, a começar pelo de abrir, e pede o teste do `Carregar`, em que a falha tira uma rodada, nunca abaixo de metade da Essência. Erguer de novo na mesma Expansão não tem recarga, mas custa a Ação Padrão e aguenta metade.* **O refino ficou só no raio e no gate.** *Na Cesta, o teste dos golpes passou de Vigor para o do `Carregar`. O voto, o gate, os `2` PE, a queda na hora e o que ele anula continuam os da v0.268. O porquê está na entrada da v0.269 do `CHANGELOG`.*

> ## A v0.268 reescreveu o Domínio Simples, e a Cesta Oca perdeu a recarga
>
> **Rodada 3 do item 6, a primeira das três que faltavam:** *ele cai por um relógio contra a Expansão — metade da Essência (mínimo 1) mais uma rodada, menos uma por ponto de refino que o dono dela tem acima do seu, **fórmula trocada na v0.269** —, golpe no dono não o derruba, e quando ele cai a Expansão alcança na hora quem ele protegia. Os pés viraram o voto do iniciante (refino 4; com refino 5 ele anda com você), o PE são `2` fixos, e lá dentro a Expansão não alcança ninguém.* **Contra refino 10 nenhum Simples segura até o fim**, *que é o Gojo perdendo o dele para o Sukuna.* *A Cesta ganhou a mesma queda na hora e perdeu a recarga. O porquê está na entrada da v0.268 do `CHANGELOG`.*

> ## A v0.267 reescreveu a Cesta Oca, e ela virou a base da revisão dos anti-domínio
>
> **Rodada 2 do item 6, com as decisões do Mizuki em cima da conta:** *o preço deixou de ser o turno inteiro e passou a ser as duas mãos presas no símbolo (Desarmado vale); ela levanta com Reação quando uma Expansão abre ou Ação Bônus no turno; e ela **cai pelos golpes em você** — Vigor contra a CD de quem te feriu, falhas acumulando até metade da Essência, como na corrida. A Expansão não a quebra enquanto você segura; soltando, cada Acerto letal conta uma falha; depois de cair, recarga de metade da Essência em rodadas.* **Ela segura quase tudo cedo e cede tarde, e deixou de ser inquebrável.** *As outras três passam por revisão com ela de base. O porquê está na entrada da v0.267 do `CHANGELOG`.*

> ## A v0.266 corrigiu na peça 11 o que ela atribuía à obra, e nenhuma regra se moveu
>
> **Rodada 1 da revisão dos anti-domínio, só texto:** *os "quatro preços" deixaram de "vir da obra" (só o da `Extensão de Domínio` vem), o raio de 2,21 m é o da Miwa, o Kusakabe alarga o raio na obra e a aptidão não faz isso de propósito, o Gojo parou de usar a `Pétala` depois de pegar o domínio, e ela "não para o que não é Acerto" em vez de "ataque físico" — nas peças 11 e 25.* **As regras questionadas ficam para a rodada 2, que começa pela `Cesta Oca`.** *O porquê está na entrada da v0.266 do `CHANGELOG`.*

> ## A v0.265 conferiu a pesquisa dos anti-domínio, e nenhum número do sistema se moveu
>
> **A rodada foi atrás de drawback e de como cada uma cai, e depois passou o `H` por uma verificação cega** — *`3.254` linhas novas em `01-pesquisa/anti-dominios/` (`I` a `N`), `34` páginas do mangá lidas quadro a quadro, `118` afirmações conferidas contra a fonte.* **Nenhuma das quatro tem relógio próprio: todas caem por fora.** **E duas coisas da v0.264 caíram:** *`中和` como subtração só tem cena na **Extensão** — a Cesta e o Simples protegem inteiro até cair —, e "a Extensão dilui o Acerto" perdeu a base: o Fanbook diz que ela neutraliza, e não há cena.* **As divergências com a peça 11 passaram de oito para catorze** — *seis frases da peça atribuem à obra o que ela desmente.* *O porquê está na entrada da v0.265 do `CHANGELOG`.* **⚠ Tudo isso está no branch `claude/jjk-anti-dominios-research-59hjly`, fora do `main`.**

> ## A v0.264 foi a campo nos quatro anti-domínio, e nenhum número do sistema se moveu
>
> **`6.023` linhas de nota de campo entraram em `01-pesquisa/anti-dominios/`**, *e a peça 11 ficou exatamente como estava.* **A premissa da revisão estava metade errada:** *a casca não é a variável — é a diferença de saída (`出力`) entre quem defende e quem abriu.* **O diálogo do cap. 227 virou fonte primária** e fecha quatro coisas: *o gatilho da `Pétala` é **contato**, o eixo é **simples contra complexo**, o `出力` é a variável, e a função dela é **comprar tempo**.* **E `中和` é termo químico — subtração com piso, não interruptor.** *As **oito** divergências entre a peça 11 e a fonte estão registradas na seção do item 6 da fila, e nenhuma foi aplicada: as quatro primeiras são conserto de fato, as quatro últimas mexem em preço.* *O porquê de cada uma está na entrada da v0.264 do `CHANGELOG`.*
>
> ⚠ **Corrigido na v0.265:** *o `中和` como subtração só tem cena na `Extensão`, e as oito divergências são catorze — ver o bloco de cima.*

> ## A v0.263 fechou duas das três pontas da v0.262, e aplicou a v0.216 em quinze lugares
>
> **O treino tem dois graus desde a v0.212** — *a especialização da peça 11 §3 —, e a peça 4 dizia "binário por ora".* **O ofício é da Origem desde a v0.216, e a peça 7, a Kaori e três capítulos do livro diziam que era do Caminho** — *o capítulo 8 dava mais dois em cada Caminho.* **A faixa de perícia treinada saiu do validador e passou a morar na peça 7 §7, julgando só a rota de base**, *e a Aura ganhou linha na `Base por Classe` com o raio da Explosão (manual v7.38).* **A ponta 3, a ficha, espera o commit das rodadas pendentes de lá** — *item 9 da fila.* *O porquê de cada uma está na entrada da v0.263 do `CHANGELOG`.*
...
> **O `Absorver` do Bastião contra a Reação de cobrir-se:** *ele evita de `2,79 ×` a `3,79 ×` o que ela evita, em todo nível, e é de graça.* **Não é dominância — aquele filtro mede coisa que o jogador escolhe entre, e ninguém escolhe entre essas duas —, e não são a mesma peça com dois nomes:** *o que as separa é o relógio, contador de dia contra dreno de PE.* ***Decisão do Mizuki: declarar e não mexer***, com o `Muro` do nível 19 comendo `86%` das rodadas de Reação do dia.
>
> **O `Não Sou Gente` REPROVOU, e ele nunca podia ter sido escrito.** *A lista `Limites` do manual e do capítulo 9 do livro põe "imunidade completa a um tipo de dano ou condição" entre o que nenhuma Passiva paga pode fazer; a caixa que a v0.39 citou fala de **furar** imunidade, que é o lado do ataque.* **Cento e quarenta e nove versões de decisão tomada e nunca aplicável.**
>
> **O `Desarmado` fica em `3,45`, e a régua passou a dizer o que ela mede.** *A dívida afirmava que repreçar mexeria no manual inteiro; a Melhoria `Condição` cobra pelo nível, o nível sai da banda, e ele ocupa `22%` do teto da `Leve`.* **Ele é a única das treze que uma compra feita antes da campanha desliga inteira, e isso ficou escrito** — *o molde do piso, se um dia pesar, é o `−2` na iniciativa que o `Surdo` ganhou.*
>
> **Três checagens novas, todas guardando relação entre dois donos, todas com o estado oposto declarável** — *sem ele o contra-teste não existe e a checagem fica trivialmente verdadeira.* **Sub-blocos nos três, então a contagem não se move.**
...
> **O `Coro` perdia a Trilha em definitivo no primeiro acerto de inimigo comum**, porque a régua se media contra a vida máxima daquele corpo e o corpo dele vale meia Rotina — o mesmo que um golpe de rotina. **Hoje a régua é `5 ×` a fórmula do tipo, escala fixa para qualquer corpo**, e o `Coro` não ganhou vida nenhuma: ele cai igual e volta.
>
> **O livro não publicava o dano do `Investir`, a vida do `Servo` e da `Matilha`, nem o orçamento do `Servo`.** *O dano nunca tinha sido multiplicado em documento nenhum — a peça dizia "entrega a cota da Rotina" e parava ali.* **Ele é `d6` arredondando o número de dados para baixo**, porque para o mais perto ele fura o teto de uma Rotina.
>
> **A Constituição valia `5 × o nível` num corpo de invocação e `1 × o nível` numa ficha**, porque ela entrava na fórmula e o multiplicador da Trilha multiplicava ela junto. **Hoje o corpo forte é `2,5 × (base + 2 × nível) + Con × nível`**, e ele fica em `101%` da curva da Vanguarda em Con `0`, `3` e `6`. *A vulnerabilidade da `Matilha` a área desceu de `×2` para `×1,5` no mesmo pacote, e o par ficou melhor que qualquer um dos dois sozinho.*
>
...
| Guia | d8 | 8 | 5 | 5 | 10 |
| Evocador *(fora da edição jogável desde a v0.270)* | d6 | 6 | 4 | 6 | 10 |
...
**Caminho e Trilha:** degrau de Caminho em `2 · 7 · 15 · 23 · 30`, entrega de Trilha em `2 · 11 · 19 · 27`. *O dono é a coleção v0.4, em `caminhos/`.*
...
**Vinte e três perícias e onze ofícios.** Perícia tem atributo fixo; ofício não — o atributo muda com o que você faz. O Caminho dá **duas perícias fixas + cinco livres**; a Origem dá mais duas perícias e **dois ofícios livres**. Nove de vinte e três, 39%.
...
**Invocações:** o invocador e todas as invocações somados entregam **uma** Rotina. A máquina inteira é a **peça 15**; o teto é da peça 6 §4. *Fora da edição jogável desde a v0.270, com o Evocador; o desenvolvimento novo mora em `invocacoes/`.*
...
| `03-mecanica/06-caminhos-e-trilhas.md` | a base dos cinco Caminhos e o argumento de cada um; desde a v0.270 o texto dos quatro que não são o Evocador é o da coleção v0.4 |
...
| `03-mecanica/conferir-invocacoes.py` | as **trinta e quatro** checagens da peça 15, sem um número guardado dentro dele — e a busca exaustiva das 5.429 montagens que gastam o orçamento cheio no nível 30 |
...
| `03-mecanica/conferir-marcial.py` | as **catorze** checagens da peça 20, e o par declarado entre a 3 e a 9: uma pergunta se a rota recebe demais, a outra se ela é frágil demais |
...
| `03-mecanica/27-ritual.md` | **o Ritual**: a escada de `2 · 6 · 10` pontos, a CD que rola `Ocultismo` contra `8` + Classe − Destreza, a lista própria de dezesseis, e a matriz Forma × Melhoria. *A peça inverte a regra da obra — o padrão do sistema é omitir, e ritualizar é desfazer a omissão.* |
| `03-mecanica/conferir-ritual.py` | as **sete** checagens da peça 27. *A `5` é a que ele pediu: ela não julga se a Melhoria anula Restrição — exige que cada uma DECLARE com quais ela não entra, inclusive "nenhuma". Pega erro de omissão, e achou a segunda trava na primeira rodada.* |
...
| `caminhos/` *(na raiz)* | **a coleção v0.4 dos Caminhos**: o texto do Bastião, da Vanguarda, do Emanador e do Guia e das doze Trilhas, com as ideias reservadas e as notas. *Dona do texto desde a v0.270, sem preço medido* |
| `invocacoes/` *(na raiz)* | **o subsistema de Invocações em desenvolvimento**, fora da edição jogável: a candidata r5, as pesquisas, e no `museu/` o capítulo de Invocações e a seção do Evocador que saíram do livro |
...
O manual do Fundamento **v7.38** (`manual/Fundamento-MANUAL-v7.docx`) é o subsistema de técnica e feitiço, já validado. `manual/gerador/` traz o gerador (Node: `npm install docx && node make.js`) e `manual/matematica/` os validadores `pac7.py` e `v7.py`. **O `.pdf` está na mesma versão do `.docx`, exportado junto desde a v0.93**, e ele deixou de ser exportado a mão: sai do `soffice --headless`. *Desde a v0.246 ele usa as fontes de métrica instaladas em 14/09 — Carlito no lugar da Calibri e Gelasio no da Georgia.* **A contagem de páginas, parágrafos e tabelas saiu daqui na v0.263:** *esta linha dizia `49` páginas, `366` parágrafos e `90` tabelas, e o manual da v7.37 já tinha `50`, `410` e `95`. Contagem de artefato não tem dono, e copiada ela só envelhece.*
...
6. **"O mestre declara o que foi uma luta" é discricionariedade que vira número.** *Aceito de propósito na v0.26*, e é a única coisa da peça 10 sem lista fechada por baixo — a declaração muda quantos degraus de exaustão o grupo acumula, e isso muda quanto PE o respiro devolve. A aposta é que ninguém está em melhor posição de dizer se aquilo foi uma luta do que quem acabou de dirigir a cena. Se dois mestres divergirem no playtest, o conserto é o do ambiente propício: fechar a lista.
   ...
   > ***MEDIDO na v0.259, a pedido do Mizuki, e a aposta se sustenta melhor do que o texto acima dizia.*** *A pergunta certa não é "os dois mestres contam igual?" — é **"duas fichas idênticas que jogaram a mesma sessão terminam o dia em estados diferentes?"**, e essa tem número.*
   >
   > **A divergência vive numa janela de três degraus de largura, e ela é fechada dos DOIS lados.** *Abaixo de `4` lutas contadas ninguém pega degrau nenhum, então discordar não separa nada; e a partir de `6` o teto empata os dois — quem conta `6`, `8`, `11` ou `18` fica no degrau `3` igual.* **Só entre `4` e `6` a contagem move a ficha.**
   >
   > **Numa discordância realista a separação é de `1` a `2` degraus.** *Numa sessão de `5` cenas de combate, um mestre contando `4` e outro `5` separam `1` degrau e `0` pontos percentuais de PE; contando `3` e `5`, separam `2` degraus e `10` pontos percentuais.* **A separação máxima de `3` degraus exige discordância total** — *um mestre chamando `0` de luta e o outro chamando as `6`, na mesma sessão* —, e isso não é divergência de julgamento, é de regra não lida.
   >
   > ***E o achado que importa: quem faz o filtro multi-mestre passar é o TETO DE TRÊS.*** *A peça declara um motivo para ele — transformar a espiral em decisão —, e ele tem um segundo que ninguém tinha escrito: **ele satura a discordância**. Sem teto, dois mestres que contassem `5` e `12` separariam sete degraus; com ele, separam zero.* **O §4 da peça 10 passou a escrever as duas metades.**
   >
   > **A divergência também não atravessa a mesa no caso normal**, *porque a exaustão **zera** no descanso longo em ambiente propício e o §7 da peça fecha a missão na escola.* **Ela só persiste em missão de campo que vira a noite fora** — *e ali ela é a espiral que o teto já segura.*
   >
   > **Fica aberto, e é menor do que o item parecia:** *o mestre pode **tirar** um degrau quando a ficção pedir, e essa é uma segunda discricionariedade, que anda em cima do resultado já contado.* **Ela só anda para o lado do jogador** — *o mestre nunca acrescenta degrau fora da regra* —, então ela não pode piorar uma ficha, só melhorar. *Continua sendo pergunta de playtest, e continua sem número.*

7. **A remodelagem das invocações e do Evocador.** *Anunciada pelo Mizuki na v0.246 como a revisão das invocações, e confirmada em 19/09/2026: "as invocações e o evocador vai ser remodelado".* **A v0.251 fechou a CD dos efeitos e deixou três coisas para ela**, *e a v0.258 mexeu em duas:*

    - ~~*a invocação com arma — quem a treina, e se o dado da arma soma ao `Investir`*~~ — ***FECHADA na v0.258, e as duas metades eram derivadas:*** *o dado **não soma**, porque o `Investir` já entrega a cota inteira da Rotina e a melhor arma de tiro somaria `314%` dela no nível 2, contra o teto de uma Rotina que é a trava mais dura da peça 15; e a invocação **nunca tem treino**, porque não tem Caminho, o que cai na penalidade de empunhar sem treino da peça 19 §6.* **A CD continua não mudando com a arma — só o acerto segue o atributo dela.** *A remodelagem pode mexer nas duas; o que ela não precisa mais é destravar a mesa.*
    - *as quinze entradas do catálogo, das vinte, que têm texto diferente entre o livro e a peça — a `Montaria` e o `Remoto` mudam regra.* **Continua aberta.**
    - *a `Voz`, cuja troca no nível 7 para `metade da maestria` não muda número nenhum, porque o piso de `1` a deixa em `+1` até o nível 25.* **Continua aberta.**

    **E a v0.258 pôs uma quarta na fila dela, por decisão do Mizuki:** *a **invocação que não obedece** — a Rika e o Mahoraga, que a peça 15 §3.4 e §3.6 dizem **duas vezes** que "precisam de tratamento antes de a peça fechar", e ela fechou na v0.58 sem o tratamento.* **É a única das três marcas do capítulo 16 que sobrou**, *e ela é desta fila porque uma invocação que age sozinha mexe justamente no teto de uma Rotina.*

    **Nada disso trava a mesa:** a CD, o atributo e o Teste de Resistência de cada efeito já estão escritos.

8. ~~**Força sem peso.**~~ **FECHADO na v0.256.** *Levantado pelo Mizuki em 19/09/2026: "Dar equivalencia de peso pro sistema, não da pra n ter sistema de peso nele".* **Hoje o livro só diz que a Força serve para "carregar" (a tabela dos atributos, capítulo 1), e não existe escala de peso em lugar nenhum:** *a Forma `Efeito` tem escala de área e de gente, e a Força aparece como requisito de arma e de `Traje`, mas nada diz quanto peso um valor de Força levanta, arrasta ou arremessa.* **Cuidado com o nome: `Carregar` já é Restrição e ação.**

   > ***DECIDIDO pelo Mizuki em 19/09/2026 (conversa da v0.253), e ainda NÃO aplicado.*** **1 · Pra que o peso serve:** *só o limite do quanto se consegue carregar e os requisitos de arma e uniforme; "de resto n vai servir pra mais nada, igual DnD 2024".* **Sem degraus de carga e sem penalidade por item.** **2 · A curva:** *ele mandou pesquisar a comunidade e ver a opção que mais gostam; se não desse conclusão, seguir o Pathfinder, e misturar com o D&D: a ficha calcula os itens em Volume, e existe o cálculo em quilos para o que não tiver Volume registrado, por decisão do mestre.*
   >
   > **A pesquisa de comunidade NÃO deu conclusão, e isso está dito aqui.** *Achei duas conversas pequenas no fórum EN World (dez respostas cada, uma delas sem ser enquete): as preferências se dividem entre peso em quilos, Volume abstrato do Pathfinder e casas de item (slots), e o ponto em comum é que ninguém quer somar peso item a item; blogs dizem que o peso do D&D é "ignorado na maioria das mesas" e que o Volume do Pathfinder "é feito pra ser usado", mas são fonte fraca. O Reddit não é alcançado pela busca.* **Então vale a regra dele: seguir o Pathfinder, misturado com o D&D.**
   >
   > **O que os três sistemas dizem, lido dos PDFs dele:** *D&D 2024, Médio: carrega Força × `7` kg e arrasta, empurra ou levanta Força × `13,5` kg (o dobro), e passar do que carrega limita o deslocamento a `1,5` m.* *Pathfinder 2e: sem penalidade até `5` mais o modificador de Força em Volume, máximo `10` mais o modificador; item de `2,5` a `5` kg é Volume `1`, menos de meio quilo é desprezível, e `10` itens leves valem Volume `1`; a pessoa Média tem Volume `6`, o que dá uns `12` kg por Volume, o dobro ou mais do que o item.* *GURPS 4e: Base de Carga é ST × ST ÷ `10` kg, em cinco degraus até `10` vezes a Base.* **Achado: o Volume do Pathfinder não é linear com o quilo (item `2,5–5` kg, pessoa `~12` kg), então o quilo do modelo misto precisa de um fator escolhido.**
   > **A calibração do sistema:** *a peça 2 diz Força `1–2` = pessoa comum, `3` = bom, `6` = o topo humano; isso põe o topo em D&D Força `20` (carrega `140` kg, arrasta `270`) e o comum em `70` kg.* **Falta:** *o limite em Volume por Força, o fator kg por Volume, o Volume de cada item do catálogo de arma e de uniforme (hoje eles têm só "requer Força", que continua), e o campo na Ficha Pessoal.*
   >
   > ***DECIDIDO pelo Mizuki em 19/09/2026, segunda rodada.*** **1 · O fator é `12` kg por Volume, e o limite fica em `60` a `132` kg** *(Volume `5 + Força`, de Força `0` a `6`; arrastar, empurrar e levantar é o dobro).* *"Num geral a força vai mais ajudar a definir kit e afins."* **E isso deve permitir tirar o limite de armas de grau, e a sintonização junto, "já que arma vai ter peso, já vai ser o limite necessário".** **2 · Os requisitos de Força de arma e de uniforme ficam INDEPENDENTES do Volume.** *"O peso (volume) de cada item/arma vai servir como um segundo balanceador"*, *e isso abre repensar as armas e acrescentar as que estão faltando.* **Falta ver o que o limite de grau e a sintonização fazem hoje, antes de decidir tirar.**
   >
   > **O que o limite de grau e a sintonização fazem hoje, lido das peças 16 e 20 em 19/09/2026:** *o `Teto de Estigma` é `3`: a arma tem teto pelas mãos e o objeto de apoio tem teto de dois; ele é estrutura do corpo, e a peça 20 diz que "conta pelas mãos e não pela mochila" — três armas guardadas com uma empunhada continuam sendo uma. Um `Estigma` de Classe `3` é do tamanho de uma aptidão, e `3` deles são `43%` das sete escolhas de marco da campanha (`1` são `14%`, `4` são `57%`). A sintonização é outra coisa: é o ATO de ativar, um descanso curto por ferramenta; sem ela a ferramenta é arma comum.* **Então o peso, que limita a mochila, não substitui o teto:** *a arma já é limitada pelas mãos, e o objeto de apoio (anel, cordão) pesa quase nada e não seria limitado por Volume nenhum.* **O peso limita a VARIEDADE que se carrega para trocar, e é aí que a sintonização hoje segura.** *Decisão dele pendente: tirar só a sintonização, ou mais.* **Também anotado por ele: o Volume de cada item vira um segundo balanceador, e isso abre revisar o catálogo de armas e acrescentar as que faltam** *(hoje são `52`).*
   >
   > ***DECIDIDO pelo Mizuki em 19/09/2026, terceira rodada (Estigma e sintonização).*** *Resposta literal: "Mantem limite pros de apoio, reduz pra 2. Mas remove o limite para arma e tira a sintonização".* **Leitura minha, para ele corrigir:** *o objeto de apoio continua limitado a `2` (que já é o valor hoje, peça 16 §5: "o apoio tem teto de duas"); a arma com `Estigma` deixa de ter limite de quantidade (as mãos continuam sendo físicas, e o peso limita o que se carrega); e a sintonização deixa de existir: a ferramenta funciona enquanto se empunha ou se veste.* **Isso reverte a decisão da v0.144 (sintonizar por descanso curto).** **O que precisa mudar quando for aplicado:** *peça 16 (§1.1 e o §5), capítulo 55 do livro, o `conferir-ferramenta.py` (a checagem `16`, de sintonia, e a `6`, do teto), e a peça 20 §4 com a checagem `11` do `conferir-marcial.py`, que fixa "o teto não se moveu".* **Aberto:** *o poder que a variedade de armas trocadas solta não foi medido; hoje a conta de Estigma é "quantos ao mesmo tempo".*


   > ***ESCRITO na v0.256, e a conta corrigiu DUAS leituras minhas pelo caminho.***
   >
   > **1 · O `Volume` de cada item não foi escolhido a mão.** *A régua lê as propriedades que a peça 14 §5.3 já publica: `de duas mãos` = `2`, `de uma mão com `Oculta` ou `Vestida`` = leve, todo o resto = `1`. Isso dá `17` leves, `14` de `Volume` `1` e `21` de `Volume` `2`, e o validador reconstrói os três baldes do catálogo em vez de guardar os números.* **A `Volumosa` NÃO entrou na régua:** *ela já é desvantagem vendida por orçamento — o `Nodachi` compra `Rompe` pagando com ela —, e o que ela descreve é ocultação e espaço apertado, não peso. Cobrar `Volume` em cima cobraria duas vezes.*
   >
   > **2 · O uniforme veio de validação externa, que o Mizuki pediu:** *"Faça uma validação baseado em outros sistemas, o quanto normalmente uma armadura pesada come do limite de uma ficha e afins, vamos na média".* **Os cinco sistemas medidos se partem em DOIS desenhos, e a fração depende do desenho:** *limite único (D&D 2024 `29%`, Pathfinder 2e `44%`) contra degraus de carga (D&D 5e variante `87%`, Pathfinder 1e `76%`, GURPS 4e `74%`).* **O nosso é o de limite único, e são os dois que ele mandou misturar: a média é `36%`, que de `Volume` `11` dá `4`.** *`Traje` `L · 1 · 2`, `Revestimento` `2 · 3 · 4`, escudo `L · 1 · 2`. A escada sobe sem tropeço: `2%` · `20%` · `25%` · `25%` · `33%` · `36%` do limite que a Força exigida por cada peça dá.*
   >
   > ***3 · CORREÇÃO: o peso NÃO substitui o `Teto de Estigma`, e a primeira leitura desta versão dizia que sim.*** *O teto da peça 16 §5 é de MÃO — uma arma empunhada mais dois apoios — e a peça 20 §4 escreve "conta pelas mãos e não pela mochila".* **Então a arma com `Estigma` já não tinha limite de quantidade guardada, e a decisão dele de "remover o limite para arma" não removeu nada que existisse.** *O que o peso substitui é a **sintonização**: era ela que segurava a variedade carregada, cobrando um descanso curto por ferramenta.*
   >
   > ***4 · CORREÇÃO: as checagens `6` do `conferir-ferramenta` e `11` do `conferir-marcial` NÃO precisaram mudar***, *contra o que este item previa: as duas guardam o teto de mão, e o teto não se moveu.* **A `16` foi reescrita inteira** *(empunhar ou vestir, com a regra antiga proibida — e a proibição é diferente nos dois documentos: a peça pode falar de sintonização, porque é registro de design, e o livro não, porque é texto de mesa).*
   >
   > **5 · A camada `3` do §6 da peça 14 ligou, e não foi o gatilho dela que disparou.** *Aquela seção dizia "se o playtest mostrar que o grupo leva tudo que quer sem precisar escolher, o espaço entra", e `04-playtest/` continua vazia. Quem ligou foi ele, e isso está escrito na peça para ninguém ler daqui a dez versões que o playtest aconteceu.*
   >
   > **Fica para depois, por decisão dele em 19/09/2026:** *revisar o catálogo de armas e acrescentar as que faltam.* **E o `conferir-equipamento.py` ganhou a checagem `15`**, *que deriva o limite, a régua, os três baldes, a escada do uniforme e o `4` do `Revestimento` `3` — sem número escrito no código.*

9. ~~**Concentração sem Restrição, e duração longa só na Forma `Efeito`.**~~ **FECHADO: a Concentração na v0.253 (Vigor contra a CD de quem te feriu), e a duração na v0.254 (a `Concentrada` e a `Duradoura`, na Família `Tempo`).** *Levantado pelo Mizuki em 19/09/2026: "a gente n abordou uma restrição pra por concentração e nem formas de aumentar duração de feitiços como minutos, horas e talz, q nem dnd, q são os feitiços mais 'especiais' e n de dano".* **Até a v0.252:** *a Concentração era regra do capítulo 2 (um efeito por vez, Vigor contra `10` ou metade do dano; desde a v0.253, Vigor contra a CD de quem te feriu), a Melhoria `Fica` dá `1` minuto e exige concentração, e minutos, horas, dia e semana existem só na tabela da Forma `Efeito`, por Classe.* **Falta:** *uma Restrição que ponha concentração num feitiço em troca de pontos, e um jeito de estender duração fora da Forma `Efeito`.*

   > ***DECIDIDO pelo Mizuki em 19/09/2026.*** **A metade da Concentração FECHOU na v0.253; a metade da duração segue aberta.**
   >
   > **1 · A Concentração rola Vigor contra a CD de quem te feriu, um teste por golpe** *(peça 3 §3, subseção `A CD de quem te feriu`).* **Por que um por golpe:** *o jogador raramente enfrenta mais de duas fichas, e o inimigo tem que bater em vários alvos.* **O motivo da troca:** *a CD antiga (`10` ou metade do dano, sem teto) quebra no dano de chefe — no nível `10` ela passa a `18` e quem treinou o Vigor resiste `45%`, e do nível `15` em diante nenhum `d20` resiste.* **A CD do inimigo é `8 + atributo + maestria` (peça 26 §3.1): `12` nos níveis 2 a 9, `14` nos 10 a 17, `16` nos 18 a 25, `18` nos 26 a 30.** *Contra ela, quem investiu e treinou o Vigor resiste `65%` em todo nível; quem não treinou cai de `60%` para `45%`, e Constituição `3` sem treino cai de `60%` para `30%`.* **Chefe com `2` golpes por rodada a `50%`, e `1` golpe em `4` em quem concentra: segura `10` rodadas em `41%` em qualquer nível, contra `53%` no nível `5`, `24%` no `10` e `7%` do `15` em diante na regra antiga.** *As duas tabelas e os números da prosa saem dos donos: é a checagem `7` do `conferir-acao.py`, sem número escrito no validador.*
   >
   > **2 · O `Carregar`** *(preço `Média`, Teste de Resistência de Espírito)* **usa a mesma CD de quem te feriu.** *A `Mão Firme` (dano de `10` ou menos) continua como está, e a `4m` do `conferir-manual.py` lê o `10` do `.docx` e compara com as três outras cópias.*
   >
   > **3 · A corrida de domínios virou a mesma rolagem, só com a contagem trocada.** *Isso reverteu a decisão de 12/09, que dava a CD do dono do outro domínio: o Mizuki alinhou em 19/09, na pergunta que eu fiz antes de mexer.* **Quando o jogador acertou o inimigo de mais de um jeito na rodada, com CDs diferentes, vale a maior** *(default meu, para ele vetar).*
   >
   > **Defaults meus, escolhidos com ele em 19/09:** *golpe de arma ou desarmado usa `8 + o atributo do ataque + maestria` (a peça 26 já lê assim a CD do inimigo); dano sem autor (queda, armadilha) tem a CD declarada pelo mestre na escada da peça 4 §2; a invocação usa a CD dos efeitos dela (peça 15 §3.6).*
   >
   > **O que a regra cobra, e ele ainda não viu:** *nos níveis baixos ela é mais dura que a antiga para quem concentra (`41%` contra `53%` no nível `5`), e um golpe pequeno de inimigo forte pede o mesmo teste que um golpe grande — só a `Mão Firme` livra dos golpes de `10` ou menos.* **E a escolha de um teste por golpe quase não pesa no número:** *contra `1` golpe em `4` são `41%` por golpe e `42%` por rodada, e se o chefe foca em quem concentra são `2%` contra `5%`.*
   >
   > **Onde a regra mora, medido na v0.253 e não de memória:** *peça 3 (dona), `partD.js` (o `Carregar`) e `partE.js` (a corrida) do manual, capítulo 2 (`11-o-turno.md`), capítulo 9 (`40-fundamento.md`: o `Carregar` e a corrida), glossário, e o texto compilado do livro.* ***As peças 14 e 24, o `dados.js` do gerador de inimigo e o capítulo 8 (`35-caminhos`) NÃO carregam a regra*** *— os `concentra` ali são "o chefe concentra o fogo", o foco de dano, e não a Concentração.* **A ficha (`Claude 3`), conferida nos arquivos dela:** *o catálogo em JSON dela tem o `Carregar` com a CD antiga (das duas ocorrências de `concentra` ali, a outra é a `Fica`, que não muda), e o texto do manual dela tem quatro cópias da regra antiga — capítulo 2, `Carregar`, corrida e glossário.* **Só mexe depois do livro.**
   >
   > **A DURAÇÃO — o que falta:** *a regra mais duas Melhorias, e não Restrição. O efeito de duração em combate exige concentração, com uma Melhoria de duração para o feitiço comum e outra para tirar a concentração, que custa mais; a Forma `Efeito` fica como está.* **Falta precificar as duas.** *Da pesquisa que ele já fez: D&D não tem preço de duração; com concentração a duração é quase de graça, sem concentração ela custa nível (`1` minuto `2,0`, `1` hora `3,1`, `24` horas `4,7`, até dissipar `5,4`); a escada da Forma `Efeito` (Classe `1` = 1 min, `2` = 10 min, `3` = 1 h, `4` = 1 dia, `5` = 1 semana) já bate com a do D&D, e a Melhoria `Fica` (`Média`) custa `1` ponto na Classe `1`.* **É a v0.254.**
   >
   > **O custo real da concentração, medido em 19/09/2026 para precificar a Melhoria que a tira:** *sob a regra nova, contra o cenário da peça 3 (`2` golpes por rodada a `50%`, `1` em `4` em quem concentra), num efeito lançado na rodada `1` de uma luta de `3` a `4` rodadas, quem treinou o Vigor mantém o efeito em `~90%` da luta (desconto de `10%`), e quem tem Constituição `3` sem treino no nível `30`, `~81%` (`19%`).* **Se o chefe foca em quem concentra, o desconto sobe a `34%` para quem treinou e a `52%` para Constituição `3` sem treino no nível `30`.** *Ou seja, tirar a concentração vale de `~10%` a `~35%` do efeito, e só passa disso contra quem não treinou o Vigor e é focado.* **Não é o valor de "concentrar só em um por vez", que é outro custo, e que esta conta não mede.** *A conta está feita em rascunho e ainda não virou checagem.*
   >
   > **A pesquisa de campo em outros sistemas voltou em 19/09/2026** *(1 agente, `185k` tokens; relatório no HD, na pasta de agentes de 19/09/2026 (fora dos repositórios), no arquivo RESULTADO da subpasta concentracao; 14 sistemas, 8 lidos em fonte primária e 5 só por resumo, marcados `NÃO VERIFICADO`).* **Achados que importam:** *só o d20 3.x e o 5e fazem a CD subir com o dano (Pathfinder 1e: `10 + dano + nível da magia`), então "um teste por golpe contra a CD de quem feriu" já está do lado dos que não escalam; o M&M 3e dá o desconto da concentração em número — o flaw Concentração vale `−1` por grau num efeito sustentado de `2` por grau, `−50%`, e tirar a concentração custa o DOBRO; o GURPS não derruba o efeito na falha, ele o congela como estava, e só a falha crítica encerra.* **Contra o nosso número:** *o desconto real da nossa concentração é de `10%` a `35%`, bem menor que os `50%` do M&M, então tirar a concentração deve custar de `1,1×` a `1,5×`, e não `2×`.* **Tabela de preço por Classe, lida do manual (Leve · Média · Pesada = `1·1·2`, `1·2·3`, `2·3·5`, `2·4·6`, `3·5·8`, `3·6·9`, `4·7·11`):** *A = Leve e B = Média dá razão `1,00×` na Classe 1 (empata, e a B domina a A) e `1,5×` a `2,0×` nas outras; A = Leve e B = Leve + `1` ponto dá `2,00×`, `2,00×`, `1,50×`, `1,50×`, `1,33×`, `1,33×`, `1,25×`, sem dominância em nenhuma Classe; A = Leve e B = Pesada dá `2,0×` a `3,0×`.*
   >
   > ***DECIDIDO pelo Mizuki em 19/09/2026 e APLICADO na v0.254.*** **1 · Preço: `B = Média`** *("fica mais fácil de organizar, evita de jogadores se perderem nas informações por ter uma exceção"), com `A = Leve`.* **Ele foi avisado de duas coisas e não mudou:** *na Classe `1` Leve e Média custam `1` ponto, então a `A` vira opção morta ali (não pede regra, é consequência da tabela), e o preço achatado paga `26%` a `71%` do valor de deixar uma condição durar a luta inteira, supondo o valor linear nas rodadas.* **Ele também foi avisado de uma alternativa de degrau (`A` Média, `B` Pesada) e não a escolheu.** **2 · Escopo: o feitiço inteiro.** **3 · Nomes: `Concentrada` (dura com concentração) e `Duradoura` (dura sem concentração)** *("fica intuitivo; se achar que não fica, podemos mudar").* **4 · A duração depende do TIPO do feitiço:** *buff ofensivo e debuff: `1` minuto; buff defensivo (proteção): `10` minutos; feitiço especial narrativo, sem efeito mecânico: `1` h, `8` h, `12` h, `24` h e assim por diante.* **Falta:** *classificar as Melhorias de estado do catálogo em ofensivas e defensivas, dizer como a escada narrativa (`1` h · `8` h · `12` h · `24` h) convive com a escada da Forma `Efeito` (`1` min · `10` min · `1` h · `1` dia · `1` semana), e precificar o efeito de `10` minutos, que passa da luta.*
   >
   > **A duração do buff defensivo, validada no 5e.tools em 19/09/2026** *(557 magias únicas de todas as fontes menos a `AU`, com a versão 2024 preferida quando existe nas duas; `33` proteções curadas por mim, `10` delas sem tag automática e classificadas por leitura).* **Com concentração (`19` proteções): `1` minuto `9` (seis delas de 5º círculo ou mais), `10` minutos `7`, `1` hora `3`. Sem concentração (`14`): `1` rodada `2`, `1` minuto `2`, `1` hora `5`, `8` horas ou mais `5`.** **A dúvida dele ("o Pele-Rocha não é nível baixo") se confirmou pela metade:** *sem concentração a duração sobe com o nível — correlação de postos de `0,36` em `134` magias e `0,64` nas `14` proteções, mediana de `1` h nos círculos `1` a `3`, `8` h no `4` e `24` h do `6` ao `8`; com concentração o nível NÃO explica a duração — correlação `0,02` em `235` magias, mediana entre `1` e `10` minutos em todo círculo.* **Pele-Rocha é de 4º círculo com concentração de `1` hora; as três proteções de `1` hora com concentração são todas resistência a dano, de círculo `3` e `4`.** *Entre as proteções com concentração a correlação é `−0,41` (`18` magias, amostra pequena): as mais fortes são as mais curtas.* **Leitura:** *a `Duradoura` deve escalar com a Classe, como a escada do `Efeito`; a `Concentrada` depende do efeito e não do nível.* **Guarda é o Escudo da Fé (concentração, `10` min) e Divide é o Vínculo de Proteção (`1` h, sem concentração).** **Categorias de efeito nas `33`: resistência `11`, imunidade a condição `8`, defesa `6`; o catálogo só tem defesa e dividir dano.** ~~*Resistência a tipo fica registrada como candidata a Melhoria nova, e é peça à parte.*~~ ***ADIADA na v0.258, por decisão do Mizuki: "abandona por enquanto, e espera o playtest" — o item 15 tem as contas.***
   >
   > ***DECIDIDO pelo Mizuki em 19/09/2026, fecho da duração.*** **1 · A `Duradoura` sobe pela escada `1` h · `8` h · `24` h por Classe** *(o mapa de Classe é meu: Classe `1` e `2` = `1` h, `3` = `8` h, `4` em diante = `24` h, a partir da mediana do D&D sem concentração com Classe `c` ≈ círculo `2c − 1`, e ele aceitou).* **2 · A escada de horas vale só para o buff MECÂNICO** *(resistência, imunidade, PV, condição, divisão de dano, fórmula de defesa).* **O buff NUMÉRICO (bônus fixo de defesa ou de salvaguarda) não sobe pela escada:** *o dado é que nenhum bônus de `+2` ou mais dura mais de `10` min no D&D, e as duas exceções longas são de número pequeno (`Vínculo de Proteção` `+1` por `1` h, `Auxílio` `+5` de PV por `8` h).* **3 · Ideia dele para o `Guarda` (que é o Escudo da Fé): `10` min com concentração e `1` min sem** *("daria pra por que 1min sem concentração, 10min com")* — **no buff numérico a `Duradoura` dura MENOS que a `Concentrada`, e o que ela compra é não concentrar.** **4 · PV sai do "numérico" e vai para o "mecânico"**, *porque `Auxílio` (`8` h) e `Falsa Vida` mexem em PV e duram horas.* **Defaults meus, APROVADOS por ele em 19/09/2026:** *a duração vale para os efeitos de ESTADO do feitiço (condição e buff), e não para a `Fica`, o dano nem o `Anteparo`; feitiço com efeitos de tipos diferentes dura a menor duração entre eles; buff ofensivo e debuff duram `1` minuto nas duas Melhorias; e a Família é `Tempo`.*

10. ~~**`Passiva Livre` e `Regra Própria` na criação.**~~ **FECHADO na v0.252, opção X.** *Levantado pelo Mizuki em 19/09/2026: falta dizer o nível máximo que a Regra pode ter na criação do Fundamento, e ele quer permitir que o Fundamento comece com os dois, porque viu ideias criativas e achou ruim ter de negar.* **Hoje o livro se contradiz um pouco:** *o capítulo 9 lista a `Passiva Livre` e, "se a técnica pedir", uma `Regra Própria` na criação; o capítulo 6 lista só a `Passiva Livre`; a tabela das Passivas abre a Classe Passiva `1` no nível `1`, `2` no `7` e `3` no `13`; e a frase da `Regra Própria` diz que só ela pode ser comprada em Classe Passiva `1` desde o nível `1`.* ***DECIDIDO em 19/09/2026, opção X:*** **a `Regra Própria` de Classe `1` vem de graça na criação, não gasta espaço, e o teto dela na criação é Classe `1`.** *Subir para a Classe `2` custa `1` espaço e para a `3` custa `2` (a diferença para a Classe `1`), então a Regra até a Classe `3` sai por `2` espaços em vez de `3`. Poder de graça: `1` espaço, que pesa `33%` da lista no nível `2` e `4%` no `30`. A opção Y, a Regra inteira grátis subindo com o nível, ficou recusada: chegava a `30%` da lista no nível `13`.* **Escrito na v0.252:** *o `partB.js` do manual (v7.33), o capítulo 9 e o capítulo 6 do livro e a peça 8, com a checagem `4l` do `conferir-manual.py`. Dois defaults meus para ele vetar: a Regra não conta nas cinco Passivas pagas, e a frase antiga "só ela pode ser comprada em Classe `1` desde o nível `1`" sai, porque a tabela já abre a Classe `1` para todas.*

11. ~~**Ritual.**~~ **FECHADO na v0.262, e é a peça 27 mais o capítulo 13 do livro, `46-ritual.md`.** *Levantado pelo Mizuki em 19/09/2026: "A gente n fez sistema de sinais de ritual para feitiços e talz".*

    > ***ESCRITO na v0.262.*** *A régua que eu usava estava errada duas vezes e ele pegou as duas — o gargalo é PE e não ação, e a comparação certa é contra gastar um feitiço para buffar (`0,59×`), não contra a rodada perdida.* **A escada virou `2 · 6 · 10` PONTOS por decisão dele, porque tier escala com a Classe e ponto de ritual não.** *As decisões e as contas estão na entrada da v0.262 do `CHANGELOG`; a regra mora na peça 27.*
    >
    > **1 · O que é.** *Correção dele sobre a minha primeira leitura: ritual **não** é conjuração lenta fora de combate.* **"Rituais são uma mecanica a parte no jujutsu que foi pouco trabalhada. O personagem principal que trabalhou rituais foi o proprio sukuna, ele usava no meio do combate para inflar, amplificar e melhorar seus feitiços."** *É ação de combate cara, e é **um capítulo inteiro só pra ele**.*
    >
    > **2 · O que ele compra: fura o limite de Melhorias por feitiço**, *que hoje é `2` nas Classes 1 e 2, `3` nas 3 e 4 e `4` da 5 em diante.* **E ele vende de uma LISTA PRÓPRIA, só acessível em ritual** — *decisão dele, e a conta obriga: uma rodada perdida vale `2,2` Melhorias `Pesada` em dano, e o limite de Melhorias segura **largura** e não dano (medido na v0.255).* **Vender Melhoria comum por uma rodada seria negócio que ninguém faz.**
    >
    > **3 · A escada de tempo tem três subidas, e DUAS JÁ TÊM PREÇO NO MANUAL:** *Ação Bônus (a Melhoria `Rápido`, que custa `Pesada`) → Ação Padrão (o normal) → Ação Completa (a Restrição `Atrasar`, que devolve `Média`) → dois turnos (**não existe**).* **Cada subida vale o que aquele degrau de tempo já custa:** *em Melhorias `Leve`, as duas primeiras compram `1` a `3`, que é exatamente a escala do F&M 2.5 — e os dois sistemas não se olharam.* **A terceira compraria de `3` a `12`, e é por isso que ela abre efeito exclusivo em vez de vaga.**
    >
    > **4 · O nome do degrau de cima é `Recitação Prolongada`**, *escolha dele.* **Passou na triagem, e as duas palavras separadas também.** *⚠ Duas coisas que a triagem NÃO pega e que precisam de cuidado ao escrever: ele é `<substantivo> <adjetivo>`, que é o formato que faz o `conferir-nomes` ler o adjetivo como nome de categoria; e `Prolongada` encosta em sentido na Melhoria `Duradoura` da v0.254 — uma é o tempo de **conjurar** e a outra o tempo que o **efeito dura**.*
    >
    > **5 · A lista tem dois eixos, os dois pedidos por ele:** *escala (*"o proprio sukuna tem diversos simbolos diferentes ao longo dos rituais que performa"*) e **Forma do feitiço** (*"vai ter melhoria de ritual q n vai encaixar por exemplo em CURA"*).* **As dez Formas já se agrupam por Família** — `Área` tem `Explosão`, `Aura`, `Cone` e `Linha`; `Amparo` tem `Cura`, `Apoio` e `Onda` —, *então o agrupamento não precisa ser inventado.* **Os três degraus abrem partes da lista, em tamanhos diferentes**, no molde do F&M 2.5.
    >
    > **6 · O acesso é uma aptidão, com gate `INT 5` e nível `10`.** ***Proposta dele*** *(*"ajudaria bastante a balancear as metrias"*), que começou em `INT 5` e nível `8`.* **O `8` foi recusado pela conta e o `10` é dele:** *no marco `6` a escolha `Corpo` dá o atributo e a escolha `Refino` dá a aptidão, e elas são exclusivas — então `INT 5` **e** uma aptidão só coexistem no marco `10`, e só para quem tem INT como principal (quem tem INT secundário chega no `14`).*
    >
    > **7 · O ritual pode ser auxiliado**, *decisão dele:* **"pessoas externas que saibam fazer rituais podem auxiliar, como no teste para suceder, amplificação e talz. Como na luta do sukuna e gojo, o gojo recebe auxilio na amplificação de seu primeiro feitiço."** *O F&M 2.5 tem o molde: o aliado gasta a Ação Completa e fornece espaços que não somam na dificuldade, e se o ritual falhar os dois pagam.*
    >
    > **A pesquisa da obra, feita em 20/09/2026:** *o Sukuna impôs uma amarra em si mesmo com **três condições** para cada Cortada — fazer o sinal de mão, recitar o encantamento do Desmantelar, e direcionar a trajetória com a palma.* **A escada de condições empilhadas é a forma que a obra já dá**, e ela encaixa no `Selo`, que é *"um gesto, um som, uma condição visível"*. *E a regra pendurada nele — **uma Restrição que cobra a mesma coisa que o Selo não devolve ponto** — é o que impede cobrar duas vezes pelo mesmo gesto.*
    >
    > **A base de referência é o `F&M 2.5 — Rituais Ritualísticos feitos por Ritualistas`**, *o sistema antigo que ele jogava, lido inteiro em 20/09/2026.* **Lá o tempo compra espaço de Melhoria** (*Ação Bônus `1`, Ação Comum `2`, Ritual Estendido `5`*), *com CD de Prestidigitação que sobe com os espaços ocupados, `Ritual Forçado` (espaço extra com `+2` de CD e colapso na falha), `Ritual Cooperativo` e a habilidade `Ritualista`.* **Ele avisou que não dá para copiar:** *"n da pra adaptar exatamente igual, principalmente pq nosso sistema é mais balanceado".* **Hoje o `Selo` é o único gesto de conjuração:** *um só por técnica, sempre o mesmo, sem custo e sem bônus, e a Condição `Calado` impede o que precisa de voz.* **Não existe conjuração de ritual, com sinais próprios e tempo de minutos, para o feitiço especial:** *o mais lento que há é a Restrição `Atrasar` (a rodada inteira) e o `Carregar`.*

12. ~~**`Alvo de Caça`, a Melhoria de buff de dano, o Hex do nosso sistema.**~~ **FECHADO na v0.255, na Família `Marca`.** *Proposta e nome do Mizuki em 19/09/2026, depois de aprovar os três defaults da duração:* **uma Melhoria auxiliar `Leve` que dá `1d4` de dano extra em cada golpe dele contra UM alvo marcado, até o fim da próxima rodada, com `1d8` quando pareada com o `Rápido`; para durar `1` minuto, a `Concentrada`.** *Ela segue a regra das Auxiliares: num feitiço de dano, o buff entra quando o feitiço acerta ou o alvo falha o Teste de Resistência; num feitiço só auxiliar, entra direto.* ***Lacuna confirmada:*** *o catálogo não tem nenhum buff de dano — `Marca` dá vantagem, `Ecoa` dá vantagem ao aliado, e `Remate`, `Acúmulo` e `Queima` são do próprio golpe.*
   >
   > ***DECIDIDO pelo Mizuki em 19/09/2026: o dado é FIXO, sem escala por Classe, e a conta confirma.*** *Dele: "no D&D a gente não tem esse aumento ao longo dos níveis, sinto que isso vai fortificar a possibilidade de só atacar ao invés de usar feitiços". **Eu tinha proposto `X`d4 (`X` = metade da Classe) e estava errado, e o erro foi da minha conta:** ela comparava o dano extra do buff COM taxa de acerto contra o dano de dado SEM ela, e subestimava o buff em `1/0,55`. Refeita com a mesma régua dos dois lados (`1` ponto = `1d8` = `4,5` de dano cheio; a Rotina é dano cheio):* **o `Xd4` paga `167%` do preço em toda Classe (`186%` com a `Concentrada`), sem nunca cair, e no nível `30` põe `20` de dano por rodada, `18,5%` da Rotina, por uma Melhoria `Leve` — mais que o `Acúmulo` (`Média`, `+3d8`, `13,5`) — e leva o golpe simples de `47,2%` para `55,5%` da Rotina.** **O dado fixo desbota como o do D&D:** *só o buff (`3` golpes na janela) paga `167%` nas Classes `1` e `2`, `83%` nas `3` e `4`, `56%` nas `5` e `6`, `42%` na `7`; com a `Concentrada` (`6,7` golpes) `186%`, `93%`, `62%`, `47%`; o `1d8` com o `Rápido` (`4` golpes, pagando o `Rápido` também) `133%`, `100%`, `57%`, `50%`, `36%`, `33%`, `27%`.* **E nenhuma das opções muda a vantagem ABSOLUTA de conjurar sobre atacar:** *o buff soma o mesmo por golpe nas duas rodadas, então a diferença entre a rodada de feitiço e a de golpe fica em `15` no nível `7` e em `57` no `30`, e o invariante da peça 11 (a rodada de golpe fica abaixo da Rotina) segue com folga — o golpe sobe de `51,6%` para `58,3%` (`62,5%` com `1d8`) no nível `7`, e de `47,2%` para `49,5%` (`51,3%`) no `30`.* **O acoplamento `1d8` com `Rápido`, que eu tinha chamado de exceção escondida, é o certo:** *ele faz a ficha que ataca só com a arma pensar em como combinar no kit, que é o que ele quer, e desbota nas Classes altas.*
   >
   > **O Nível 1 (Classe `1`, `3` pontos) NÃO cabe completo.** *O Hex inteiro (`Alvo de Caça` + `Concentrada` + `Rápido`) custa `4` pontos, que uma Restrição `Leve` (`Parado`, que devolve `1`) fecharia; mas o livro limita as Melhorias por feitiço a `2` nas Classes `1` e `2`, `3` nas `3` e `4` e `4` da `5` em diante, e o Hex tem `3`.* **Ele só cabe da Classe `3` (nível `9`), e a `9` de `9` pontos.** **Nas Classes `1` e `2` a ficha escolhe: `Alvo de Caça` + `Rápido` (`3` de `3` pontos, `1d8`, janela curta) ou `Alvo de Caça` + `Concentrada` (`2` pontos, `1d4`, `1` minuto)** — *uma decisão de kit de verdade.* **Do `Hex` completo, o custo é de `83%` a `100%` do orçamento da Classe `3` em diante.**
   >
   > ***FECHADO na v0.255, e as quatro escolhas são do Mizuki em 19/09/2026.*** **Família `Marca`** *("Coloca em marca"): ela é irmã da `Marca` — a mesma janela e o mesmo alvo, dano no lugar de vantagem —, a palavra `marcado` já mora ali, e é o MESMO estado, então a `Condicional` que dispara com "marcado por você" dispara com as duas. Em `Auxiliares` a `Régua`, que é o pronto de caçador, ficaria trancada pela Família Fechada; em `Marca` nenhum dos três prontos bloqueia, e a `Sentença` e a `Banca` têm desconto.* **O gatilho** *(dele: "Qualquer ataque que não seja 'continuo', ent rajada pega, ataque extra, afins (q nem dnd)"): toda rolagem de acerto sua que acerta o alvo marcado, com cada tiro da `Rajada` contando sozinho — é o `Hunter's Mark` do D&D 2024. Feitiço de Teste de Resistência não é ataque; dano que volta em rodadas seguintes e respingo não contam; a invocação não herda.* **Sem termo novo** *(dele: "A definitivamente") — a regra fala de "duas Melhorias escritas como uma", sem batismo e sem glossário.* **O preço aceito:** *só o buff paga `111%` do preço nas Classes `1` e `2` e `28%` na `7`; a `Rajada` seguinte soma `(Classe + 1) × 2,5`, que é `20,0` de dano na Classe `7` (`18,5%` da Rotina) — a mesma curva do `X`d4 que ele recusou, só que paga também com a `Rajada` e travada em `3` Melhorias. Ele viu o número antes de escolher e manteve.* **Escrito na v0.255:** *o `partD.js` (manual v7.36), o capítulo 9 e o glossário do livro, a peça 3 §3, com a checagem `9` do `conferir-acao.py` e a `4o` do `conferir-manual.py`.*

13. ~~**Combo do jogador, e a lacuna do `Efeito Próprio`.**~~ **FECHADO na v0.255.** *Dúvida do Mizuki em 19/09/2026 (se um jogador poderia fazer uma Melhoria "sem família" que junta duas Leves numa Média), pesquisada sem mexer em nada, e ele DECIDIU: fica o "combo do jogador que conta como as partes", "menos dor de cabeça".* **A regra decidida, ainda NÃO escrita:** *o combo cobra a soma dos preços das partes e conta no limite de Melhorias como as partes (é atalho de leitura na ficha, sem efeito de balanço); o texto deve deixar claro que o mestre PODE aprovar um combo custando menos ou economizando espaço, mas é decisão inteiramente dele, por conta e risco, e não recomendada.* **E isso fecha a lacuna que ele descobriu:** *o `Efeito Próprio` tem limite de um por feitiço (isso já estava escrito), mas o texto não diz se ele conta no limite de Melhorias de `2`, `3` ou `4` por Classe; a `Restrição Própria` diz que conta no limite de duas Restrições. A frase que falta é "o `Efeito Próprio` conta como uma Melhoria".* **O que a pesquisa achou (os números servem para o aviso do mestre):** *tirar o limite de Melhorias não move o pico de dano (busca exaustiva do `v7.py` numa cópia sem o limite: `17`, `34`, `51`, `68`, `84`, `101`, `118` de dano nas Classes `1` a `7`, idêntico); o que o limite corta é largura — por pontos caberiam `3`, `6`, `4`, `6`, `5`, `6`, `5` Leves e o limite deixa `2`, `2`, `3`, `3`, `4`, `4`, `4`; acrescentar a `Concentrada` sobre o buff de dano rende na margem uns `205%` do preço na Classe `2`, então o jogador monta o pacote de sinergia e os builds convergem; o D&D 2024 permite uma Metamagia por magia e só duas no nível `7` do Feiticeiro enquanto a Feitiçaria Inata está ativa; o Pathfinder 2e desperdiça o benefício de uma segunda ação metamágica.* ***ESCRITO na v0.255:*** *o `Efeito Próprio` passa a dizer que conta como uma Melhoria no limite da Classe, em três cópias de cada documento (a linha do catálogo, a frase depois da tabela de limite e a regra de ouro `3`); e o parágrafo do combo entra no `partA.js` e no capítulo 9, com a soma dos preços, o limite contado pelas partes, e o aviso de que o mestre pode aprovar por menos "por conta e risco" e que o manual "não recomenda". Sem termo novo, por escolha dele. A checagem `4o` do `conferir-manual.py` guarda as seis cópias e o parágrafo, e proíbe a redação antiga.*

14. ~~**O `Acúmulo` contra "num alvo só, feitiço comum para nos pontos da Classe".**~~ ***FECHADO na v0.259, e não por conta: por leitura de mesa.*** ***Decisão do Mizuki em 20/09/2026:*** **"o acúmulo n é um problema d verdade né, 4 rodadas é fim de combate".** *A conta abaixo fica como registro, e ela é justamente o que dá razão a ele: o caso só existe na **quarta** rodada seguida contra o mesmo alvo, e a luta média é de `3,7`.* **O que estourava era um número que a mesa não alcança.** *Achado enquanto eu media a soma do `Alvo de Caça` com o `Acúmulo` e o `Remate`, na v0.255.* **O pior feitiço num alvo só — `Acúmulo` (`Média`) mais `Remate` (`Média`) mais duas Restrições `Média` devolvendo o máximo — já passa do dano da `Liberação Máxima` nas Classes `3` e `4` na quarta rodada seguida contra o mesmo alvo:** `56` *contra* `54` *na Classe `3`, e* `73` *contra* `72` *na `4`. Isso é **antes** do `Alvo de Caça`, que soma `2,5` fixos ali (`5,6%` da Rotina na Classe `3`) e não é a causa.* **Três coisas que faltam, e é por isso que ele fica aberto:** *o manual não diz se o `+3` do `Acúmulo` é exceção do teto de um alvo só ou se o teto continua valendo por cima dele; nenhum dos `35` Fundamentos prontos usa `Acúmulo`, então o caso nunca apareceu montado; e o `v7.py` mede o dano de UM feitiço e não modela adição rodada a rodada, então a busca exaustiva dele passa por cima deste caso sem ver.* **Custa quatro rodadas seguidas no mesmo alvo para acontecer**, *que é mais que a luta média de `3,7` rodadas — é por isso que não é urgente.*

15. ~~**A Melhoria de resistência a tipo.**~~ ***TIRADA DA LISTA na v0.259, por decisão do Mizuki:*** *"o item 15 pode apagar da lista".* **A conta inteira mora na entrada da v0.258 do `CHANGELOG`**, *e ela é grande: a lacuna que motivou a candidata não existia (resistência a um tipo já é a Passiva `Escama`, desde a v0.26), o preço fecha em `72%` · `90%` · `99%` de `Pesada` para o grupo `Físicos`, e o escopo do meio não é calculável porque o Bestiário não diz quantos tipos de dano um inimigo usa.* **O nome `Amortece` passou na triagem e fica guardado lá.** *Ela sai da lista de pendências porque não é trabalho parado esperando alguém — é assunto de playtest, e o gatilho é o peso dos três grupos deixar de ser previsão.*
...
- **`Alvo de Caça` vira o loop "marca uma vez e só ataca"?** *Entrou na v0.254 e foi escrito na v0.255.* ***E a `Rajada` também dispara o dado, um por tiro***, *decisão dele em 19/09/2026 — então o loop a testar não é só "marca e bate com a arma": é também "marca e solta `Rajada`", que na Classe `7` soma `20,0` de dano, `18,5%` da Rotina.* **O dado fixo desbota nas Classes altas de propósito, e a diferença entre a rodada de feitiço e a de golpe não se mexe; se a mesa mesmo assim só marcar e atacar, o conserto medido é tirar o `1d8` do `Rápido`.**
...
| 6 | **Revisar os anti-domínios** — *decisão dele na v0.226: "vamos revisar a mecânica de como esses anti domínios funcionam"* | as quatro da peça 11, e como elas se portam contra a `Expansão sem Barreiras`. *As saídas medidas estão no rascunho, seção 8.4.* ***A PESQUISA DE CAMPO SAIU na v0.264 e foi CONFERIDA na v0.265*** — em `01-pesquisa/anti-dominios/`, com o resumo das quatro e as **catorze** divergências no `H-resumo-das-quatro.md` e a verificação cega no `N-verificacao-do-H.md`. **⚠ A premissa da 8.4 está metade errada:** *a casca não é a variável, e sim a diferença de saída (`出力`) entre quem defende e quem abriu — quatro `Domínio Simples` aguentaram quase 99 s dentro de um domínio sem casca (cap. 258), e dentro de domínio FECHADO o Sukuna pagou metade do corpo (caps. 249 e 266).* **Nenhuma das quatro tem relógio próprio na obra; todas caem por fora.** **As catorze divergências, em duas metades:** *conserto de fato — o "pés no chão" é voto da Miwa (e a obra diz "do **ponto** onde foi ativado"), o raio `2,21 m` é dela, o Kusakabe expande o raio com a própria aptidão, a `Pétala` responde ao acerto garantido que **toca** e não a "ataque físico" (a frase do Acerto corporal cai), e seis frases que a peça atribui à obra e a obra desmente: a `Cesta Oca` "não faz mais nada" e "não quebra", a `Pétala` "cai se perder a concentração" e o Gojo "nunca tinha usado", a `Extensão` "acerta independentemente", e "nenhuma serve contra a incompleta" valendo para a `Extensão`.* **Decisão de preço, e é dele** — *como cada uma cede (a `Cesta Oca` e o `Domínio Simples` protegem **inteiro até cair**, e só a `Extensão` **atenua** contra saída maior); o que a `Extensão` faz contra o Acerto de domínio (o Fanbook diz que neutraliza, e a obra não tem cena); o que separa `Domínio Simples` e `Cesta Oca` (ela segura com o selo e racha sob golpe no dono; ele cai pela pressão ou pelo voto); e o custo da `Cesta Oca`, que é **selo imposto** e veta o golpe grande do dono.* **Livres de canon:** *custo de energia, duração, `Extensão` junto com a técnica reversa e aprendizado — a obra não amarra nenhum.* **Rodada 1 aplicada na v0.266:** *as frases de texto da peça 11, e duas cópias na peça 25 — nenhuma regra mexida.* **Rodada 2 aplicada na v0.267: a `Cesta Oca` foi reescrita** — *mãos presas no lugar do turno, cai pelos golpes em quem segura (metade da Essência em falhas), recarga de metade da Essência — e ela é a base da revisão das outras três.* **Rodada 3, o Domínio Simples, aplicada na v0.268 e v0.269:** *um relógio contra a Expansão em Essência (`4`, `3` ou `2` rodadas, maior, igual ou menor que a do dono — a v0.269 trocou a fórmula de refino da v0.268), o teste do `Carregar` a cada Acerto, a queda na hora, erguer de novo com a Ação Padrão e metade das rodadas, os pés como voto do iniciante, e `2` PE fixos; a Cesta perdeu a recarga (v0.268) e passou ao teste do `Carregar` (v0.269).* **Faltam a Pétala e a Extensão, e depois a comparação das quatro.** |
| 7 | ~~**O Rescaldo e a porta de saída dele**~~ | ✅ **APLICADO na v0.239, como a aptidão `Regravação`, na peça 11 §6.** *Ação Bônus durante o Rescaldo: gasta o teto inteiro da `Energia Reversa`, a técnica volta, e fica uma marca.* **Com `metade da Inteligência + metade da maestria` marcas, a Expansão não abre até o descanso longo.** *A `Regravação` no inimigo fechou na v0.242, na peça 26 §6.4, com a corrente inteira e os marcos do jogador* |
| 11 | **O teto de atributo e o incentivo da rota `Corpo`** — *levantado por ele em 20/09/2026, enquanto mediamos a escada de CD: "o teto é alcançado por darmos de graça, oq tira o incentivo em parte de pegar rota de corpo"* | ***A premissa CAIU na medida, e fica registrado para quem voltar.*** *Só o atributo **principal** chega ao teto `6` de graça, com os sete pontos passivos dos marcos partindo de `3`. Os outros quatro só sobem se o jogador gastar a escolha — e é exatamente isso que a rota `Corpo` compra.* **A tabela da peça 2 §3 mostra: `sempre atributo` termina em `6·6·6·4·1` e `sempre refino` em `6·6·2·1·1`, dois atributos inteiros de diferença.** *E o principal chegar ao mesmo lugar nos três caminhos é declarado como proposital: "os três chegam ao mesmo lugar no atributo principal, e divergem em tudo o mais".* **As duas ideias dele ficam na fila, sem medida:** *a rota `Corpo` estourar o teto em `1` (um atributo por vez), ou o teto virar `5` com o `6` travado atrás dela.* ⚠ **Qualquer uma mexe no teto de Defesa `20`, que a peça 14 §3 deriva do teto de atributo** — *não é mudança isolada* |
| 10 | ~~**Organizar os requisitos das aptidões**~~ ✅ **FECHADO na v0.260** — *pedido dele em 20/09/2026: "tem algumas que tem niveis que não se recebe aptidão e isso n faz sentido"* | **MEDIDO na v0.259, e ele está certo: `4` das `5` aptidões com gate de nível gateiam num nível que não é marco.** *Aptidão se ganha nos marcos `6 · 10 · 14 · 18 · 22 · 26 · 30`, e a `Energia Reversa` e a `Extensão de Domínio` pedem nível `13`, enquanto o `Domínio Simples` e a `Pétala` pedem `7`.* **O efeito prático é ZERO — escrever `13` ou `14` dá o mesmo resultado, porque o marco é quem manda** —, *e o número não é arbitrário: `7` e `13` são os níveis da Classe Passiva `2` e `3`, que o §5 declara como régua herdada.* **Então o gate é inerte, não errado.** *Decidir: trocar para o marco (`7`→`10`, `13`→`14`), ou deixar e escrever que o marco manda.* **Só o `Kokusen Melhorado`, no `14`, já caía em marco.** ***FECHADO na v0.260:*** *os gates subiram para `10` e `14` na peça 11, no `DESENHO-trilhas` e na peça 25, e o guarda de título do `conferir-aptidoes.py` foi junto.* **E a troca destampou uma coisa maior:** *a peça 16 dizia herdar esse gate, e o `conferir-ferramenta.py` acendeu — a conta mostrou que o `Estigma` SEMPRE foi mais fácil que a aptidão da mesma Classe (`3` níveis na Classe 2, `1` na Classe 3), porque a peça 16 tira a metade de refino e o refino é quem segura a aptidão.* **A herança era nominal e morreu; a ferramenta fica em `7` e `13` por decisão dele** — *"Ferramenta é decisão do mestre, são exemplos a tabela… por que mexeria?"* |
| 9 | **As duas fichas novas: a `ficha pessoal` e a `ficha maldita`** — *levantado por ele em 20/09/2026, varrendo a fila: "falta nessa fila ai a criação na ficha contendo 'ficha pessoal' (aqui vai ter os itens, bio, anotações, foto, xp etc), e 'ficha maldita' (aqui vai ter as tecnicas, aptidões e afins, é uma representação maior da '8' que tem na ficha principal)"* | o repositório da ficha (`Claude 3`), e **não** este. *É ela que destrava o `Volume` da v0.256 e da v0.257, que hoje não tem onde entrar — a palavra não aparece uma vez no `Ficha.gs` nem no catálogo.* **A dívida de regra da ficha é outra coisa e é menor:** *o `Carregar` com a CD velha (o único ERRADO hoje) e três Melhorias ausentes do catálogo de consulta, que não dependem de aba nenhuma.* ***FECHADAS: a rodada da v0.258 e a da v0.260 subiram em 22/09/2026 (`629473f`).*** ✅ **A ponta 3 da v0.262 FECHOU em 22/09/2026** (`7f83e35`): *o texto que a ficha carrega — o manual em txt do repositório dela — foi reextraído do PDF de coluna única do livro da v0.263, e foi de `13.940` para `14.352` linhas.* **Ela destampou três coisas que a edição à mão tinha deixado passar:** *o `Efeito Próprio` estava com o texto de antes da v0.255; a coluna `VOLUME` da v0.256 e da v0.257 fazia as **três** tabelas de equipamento lerem ZERO; e o parser de legados lia `79` de `90` e saía verde, porque o título `Legados da Latente` caiu numa página que imprime o número na mesma linha.* **O carimbo fica em `0.258` de propósito** — *ele é a versão do catálogo, e o catálogo não ganhou nada da v0.259 à v0.263.* **As dezesseis Melhorias de ritual não entram no catálogo de consulta:** *esperam a ficha maldita, porque o ritual é montagem de feitiço e a ficha não tem bloco de feitiço.* |
| 12 | **Medir as fatias da coleção v0.4** — *v0.270, nas palavras dele: "Não é necessário medir cada fatia ou certificar novamente o orçamento para implementar. […] Isso não autoriza declarar que o equilíbrio foi comprovado. Coloque na fila que futuramente terá de ser medido."* | **os quatro Caminhos e as doze Trilhas de `caminhos/`, com o degrau novo do nível 23.** *O ponto de partida é o registro com preço da coleção anterior — os `DESENHO-*.md` e a peça 17 —, e quando a medição fechar o índice e os preços passam a ser os da v0.4. Entram junto as três pendências que a própria v0.4 registrou e o livro não fecha: o prazo e a abrangência do `Oportunista`, a ordem, o custo e o arredondamento do `Contra a Parede`, e as conversões do nível 15 do Emanador, que ela deixou sujeitas ao orçamento do construtor* |
| 13 | **O Evocador fora da edição jogável, até o subsistema dele fechar** — *decisão dele na v0.270* | *"Bloqueie temporariamente Evocador e Invocações na edição jogável."* `invocacoes/` — *a candidata r5, com decisões até o §45, parada no que acontece a zero PV.* **O texto antigo mora em `invocacoes/museu/`**, *e a peça 15 e o validador dela continuam contra ele. A volta ao livro é decisão dele, e o `conferir-invocacoes.py` e o `conferir-catalogo.py` acendem se o capítulo ou o Caminho reaparecerem antes* |
| 14 | **A ficha do `Claude 3` com os Caminhos velhos, e o catálogo do gerador daqui** | **a ficha do repositório `Claude 3`**, *que é extraída do livro e ficou com as Trilhas antigas, o Evocador e o texto velho da Cesta e do Simples até a próxima extração;* **e o `dados.js` do gerador em `05-material/`**, *que ainda lista as quinze Trilhas antigas e o Evocador como catálogo.* **As duas fichas `.docx` daqui não imprimem essa lista** *— a da Kaori só mostra a Trilha dela, o `Muro` —, e a v0.271 regerou a da Kaori para o `Provocar`.* *O `conferir-ficha.py` confere o `dados.js` contra a peça 6, que guardou a lista antiga como registro — por isso ele segue verde e não acusa a defasagem. ⚠ A v0.270 escreveu aqui que a ficha deste repositório imprimia as Trilhas antigas, e não imprime* |
| 15 | ~~**As decisões que a v0.270 deixou para ele**~~ | ✅ **FECHADO na v0.271**, *com as respostas dele: a perícia fixa do Bastião é `Provocar`; a frase dos Limites volta com o Socorrista como exceção; ninguém cura outra pessoa com Energia Reversa; e os renomes `Eco Amaldiçoado`, `Impulso Energético` (só o recurso do Catalisador) e `Sobre Carregar Energia`, com `Condutor Armado`, `Condução` e `Segunda Leitura` mantidos e toda colisão da triagem aprovada — "se a triagem colidir, pode falar que tá aprovado".* **As doze Trilhas e os três nomes novos entraram nas listas do `conferir-nomes.py`.** *Os nomes de habilidade da v0.4 entram quando a peça 17 for refeita, com a medição do item 12* |
| 8 | **Rever os inimigos, e o máximo de cada um** — *levantado por ele na v0.242: "a gente tem q começar a rever os inimigos, começar a buscar qual seria o 'máximo' deles e talz, mas isso deixa pra depois"* | a peça 26 e o bloco do Sukuna, que virou exceção declarada com o traço `Rei das Maldições` |
...
> ~~**⚠ E uma dívida de antes continua aberta:** *a coincidência dos `256` entre o PDF de coluna única e o `-A-atual`* — **e agora ela tem uma variável a menos, porque os dois artefatos usam fontes diferentes.**~~ ***FECHADA na v0.236:*** *com as fontes certas nos dois, os marcadores do PDF mostram dois capítulos a mais, um a menos e dezesseis de vinte e três com outro tamanho. Os `256` eram soma que se compensava, e hoje a coluna única tem `259`.* *A outra — a `Sobrecarga` escrevendo "o dobro de energia" — fechou na v0.221: o texto virou "ele não usa Reação", o degrau ficou `Leve` nos dois donos, e a checagem `5` do `conferir-acao.py` passou a proibir a frase nela também.*
...
| ~~2~~ | ~~**Equipamento** — armas, escudos, uniformes~~ | **fechada na v0.48** (peça 14): as 52 armas com orçamento fechado, proteção, escudo, treino e requisito de Força, mais o `conferir-equipamento.py` com quinze checagens | — |
| ~~3~~ | ~~**Invocações** — o sistema de criação~~ | **fechada na v0.58** (peça 15), com o `conferir-invocacoes.py`, que hoje tem trinta e quatro checagens | — |
...
|---|---|---|---|
| ~~1~~ | ~~**Invocações**~~ | **fechada na v0.58** (peça 15), com o `conferir-invocacoes.py`, que hoje tem trinta e quatro checagens | rotas 6/9 → 6/9 · vagas 0 de 7 |
...
**Invocações fechou na v0.58 e é a peça 15**, em `03-mecanica/15-invocacoes.md`, com o `conferir-invocacoes.py` em cima dela. O §5 daquela peça é a especificação das trinta e quatro checagens, e ele foi escrito **antes** do validador — que é por que ele coube numa versão só, contra as seis que a peça 14 gastou.
...
| ~~O **Não Sou Gente** virar Passiva paga com espaço de feitiço~~ | ***REPROVADO na v0.188, e o destino cai junto.*** *A lista `Limites` do manual e do capítulo 9 do livro põe **"imunidade completa a um tipo de dano ou condição"** entre o que nenhuma Passiva paga pode fazer.* **A caixa que a v0.39 citou fala de FURAR imunidade — `Melhoria` é peça de feitiço —, e não de tê-la.** *Cento e quarenta e nove versões de decisão tomada e nunca aplicável; a peça 13 §9.9 tem a medida e a sub-checagem `6.2` guarda a relação* |
...
| ~~**Itens iniciais por Caminho**~~ | **FECHADA na v0.211.** *Levantado pelo Mizuki na v0.135.* ***Decisão: não existe kit por Caminho, nem fixo nem recomendado*** — o orçamento livre da v0.175 já produz a variação de build que um kit substituiria, e cada Caminho só aponta pro capítulo 14. *Esta linha ficou dizendo "em aberto" por trinta e seis versões depois de fechar — é a lição nº 9 dentro da própria fila* |
| ~~**Itens menores**~~ | **TIRADA DA FILA na v0.248, por decisão do Mizuki.** *Levantada na v0.131: talismã pronto, remédio, corda, lanterna, o kit que o `Herbalismo` usa.* **A v0.171 já tinha matado a ideia de item consumível** — *"melhor não ter item consumível"* —, e ninguém tinha voltado para fechar a pendência que ficou sem substância. *A camada 1 (permissão) da peça 14 §6 segue de pé, com preço no §6.5* |
| **O que cada ofício FAZ** | *levantado pelo Mizuki na v0.139.* **Hoje os onze ofícios são só uma rolagem de análise** — o capítulo 3 diz o que cada um cobre e que o mestre escolhe o atributo na hora, e para por aí. *Falta a mecânica própria, no molde do D&D:* **o que cada um FABRICA, e o que ele faz num descanso.** *`Herbalismo` produz remédio, `Caligrafia` produz talismã, `Forja` conserta e afia — nada disso tem regra.* **Decisão dele: aqui é a versão superficial; o aprofundamento vai para o livro do mestre.** *Com **itens menores** fora da fila, esta peça é quem decide sozinha o que um ofício produz* |
