...
*Atualizado ao fim da v0.44. Copie daqui para baixo.*
...
Me diga quantos passaram **e se algum imprimiu PULADA** — verde que pulou checagem não prova nada. Sem `python-docx`, três deles pulam e saem com código 0: `conferir-nomes` pula **3 de 5**, `conferir-manual` pula **4 de 4** (todas, ele sai no `except ImportError`) e `conferir-pericias` pula **1 de 8**.
...
E o mount **às vezes some com um arquivo que ele mesmo acabou de gravar**. Sintoma: `ls` e `stat` mostram tamanho e inode certos, `open()` devolve ENOENT, os vizinhos abrem normalmente. Aconteceu duas vezes na v0.42 e **duas vezes na v0.44** — nesta última o `README.md` e o `ESTADO-ATUAL.md` caíram juntos, que é o par de sempre. O conteúdo nunca está em risco: **qualquer escrita nova reconcilia, e uma edição de uma linha basta.**
...
1. **O dado e as propriedades de cada uma das 52 armas.** A régua existe; a atribuição não. **E o dado não é eixo livre:** sob a régua ele é função de (mãos, nº de propriedades), então quem carrega a variação é a propriedade — que não é escolha, é o que a arma é. Rodado sobre as 41 de corpo a corpo, o eixo de preço dá **14 assinaturas e 85% de gêmeas**; com a categoria, **25 e 61%**. **A unicidade mora no efeito de crítico da categoria, não no dado.**
2. **Os treze efeitos de crítico**, um por categoria. Cabe até **11,0 de valor no disparo** por menos de um passo de dado, porque o 20 natural dispara em 3% das rodadas. **A armadilha documentada do PF2e:** efeito que morre contra alvo comum — sangrar morto-vivo, derrubar quem já está no chão.
...
`sistema/03-mecanica/RASCUNHO-bloqueio.md`, e agora com seção própria no `ESTADO-ATUAL`. Ela não entra em balanceamento até o tópico de regras existir, e não mudou número de peça nenhuma.
...
## Duas regras de método que custaram versão
