...
PDFS = os.environ.get('JJK_PDFS', '/media/mizuki/HD Externo II/Claude/Claude 2/PDFs - Sistemas Extras/PDF_Sistemas')
MANUAL = os.path.join(os.path.dirname(RAIZ), 'capitulos')
...
def main():
    ...
    fs = frases(do_livro())
    # ⚠ o teto original era 1000, calibrado pro Manual da Guilda. O Bestiário tem
    #   oito capítulos contra vinte e um, e mede ~615 frases. A guarda continua:
    #   ela pega corpus vazio e extração quebrada, que é pra isso que ela existe.
    assert len(fs) > 400, f'o livro extraiu {len(fs)} frases — algo quebrou'
