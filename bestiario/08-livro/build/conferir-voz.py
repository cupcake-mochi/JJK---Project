...
MANUAL = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "capitulos")
...
VERBO = re.compile(
    ...
    r"some|opera|começa|inclui|paga|sabe|percebe|usa|repetem|está|"
    # Passada 3 do Bestiário: os títulos-frase que escaparam usavam pretérito
    # ("Morreu do jeito errado", "A morte que alguém amaldiçoou") e verbos que a
    # lista não tinha ("Gente que vira maldição", "Como a mesa sai").
    r"vira|virou|morreu|amaldiçoou|matou|voltou|ficou|nasceu|sai|saiu|mora|"
    r"responde|olhando)\b",
    re.I,
)
MOLDURA = re.compile(
...
# --- Bestiário, passada 3 --------------------------------------------------
# No Manual da Guilda o `#` de um arquivo é o título do capítulo, e fica fora
# por causa das referências cruzadas. No Bestiário NÃO: o título de capítulo
# mora no build.py (listas CHAPTERS e FRONT), e todo `#` de dentro do arquivo é
# SEÇÃO. Foi por esse furo que "Morreu do jeito errado", "A morte que alguém
# amaldiçoou", "O bloco em branco" e mais vinte passaram limpos pela passada 2.
# Agora entram os três lugares onde o livro escreve título: o `#` (e abaixo) de
# dentro do arquivo, o nome de tabela ({: .tab-titulo }) e as listas do build.py.
BUILD = os.path.join(os.path.dirname(os.path.abspath(__file__)), "build.py")

# A REGRA-DE-VOZ.md dá à moldura de leitura exatamente um lugar: a introdução.
# No Bestiário ela é o 05-abertura.md, primeiro arquivo da FRONT do build.py.
# Ali a moldura aparece na lista de triagem, e não conta como achado.
INTRODUCAO = "05-abertura.md"


def codigos_de_titulo(texto, tabela=False):
    """Os códigos que um título (de seção, de tabela ou de capítulo) acende.

    Nome de tabela escapa só da regra da vírgula: "Formas, por nível" é a forma
    `<Tipo> de <Nome>` do PHB com a vírgula no lugar da preposição.
    """
    limpo = re.sub(r"[`*_]", "", texto).strip()
    # "Passo 1 · A categoria" escondia o artigo atrás do número do passo.
    limpo = re.sub(r"^Passo \d+\s*[·—–-]\s*", "", limpo)
    ...
    if PERGUNTA.search(limpo):
        cods.append("PERGUNTA")
    elif " que " in limpo or VERBO.search(limpo) or (
            not tabela and "," in limpo and " e " not in limpo):
        cods.append("FRASE")
    if CONTAGEM.search(limpo):
        cods.append("CONTAGEM")
    return cods


def titulos_do_build():
    """(linha, título) de cada capítulo e de cada parte, lidos do build.py."""
    texto = open(BUILD, encoding="utf-8").read()
    saida = []
    for lista in ("CHAPTERS", "FRONT"):
        m = re.search(r"^%s = \[.*?^\]" % lista, texto, re.S | re.M)
        if not m:
            saida.append((0, "!! a lista %s sumiu do build.py" % lista))
            continue
        base = texto[:m.start()].count("\n") + 1
        for k, linha in enumerate(m.group(0).split("\n")):
            t = re.match(r'\s*\("[^"]+",\s*"([^"]+)",\s*"[^"]*"'
                         r'(?:,\s*(?:"([^"]+)"|None))?\s*\)', linha)
            if t:
                for nome in (t.group(1), t.group(2)):
                    if nome:
                        saida.append((base + k, nome))
    return saida
...
TITULO = re.compile(r"(?m)^#{1,6}\s+`?([^`\n]{2,40})`?\s*$")  # h1 é seção no Bestiário
...
def confere(caminho):
    achados = []
    avisos = []
    introducao = os.path.basename(caminho) == INTRODUCAO
    linhas = open(caminho, encoding="utf-8").read().split("\n")
    for i, linha in enumerate(linhas, 1):
        cab = re.match(r"^(#+)\s+(.*)$", linha)
        if cab:
            # Todo nível entra, h1 inclusive: no Bestiário o `#` é seção.
            texto = cab.group(2).strip()
            for cod in codigos_de_titulo(texto):
                achados.append((i, cod, texto))
            continue
        # Nome de tabela também é título, e outro capítulo cita ele pelo nome.
        if (i < len(linhas) and linhas[i].strip() == "{: .tab-titulo }"
                and re.match(r"^\*\*.+\*\*\s*$", linha)):
            texto = linha.strip().strip("*")
            for cod in codigos_de_titulo(texto, tabela=True):
                achados.append((i, cod, "[tabela] " + texto))
            continue
        ...
        if MOLDURA.search(linha):
            if introducao:
                avisos.append((i, "[moldura autorizada, introdução] " + linha.strip()[:60]))
            else:
...
def refs_quebradas(manual, arquivos):
    ...
    for arq in arquivos:
        for linha in open(os.path.join(manual, arq), encoding="utf-8"):
            # h1 entra: no Bestiário ele é seção, e "em *Fixa e errante*" aponta para um.
            cab = re.match(r"^#{1,}\s+(.*)$", linha)
            if cab:
...
def main():
    ...
    if not filtro:
        # Os títulos de capítulo e de parte, que no Bestiário moram no build.py.
        achados_b = []
        for ln, nome in titulos_do_build():
            if nome.startswith("!!"):
                achados_b.append((ln, "FRASE", nome))
                continue
            for cod in codigos_de_titulo(nome):
                achados_b.append((ln, cod, nome))
        por = {c: sum(1 for _, k, _ in achados_b if k == c) for c in CODIGOS}
        for c, n in por.items():
            total[c] = total.get(c, 0) + n
        marca = "" if any(por.values()) else "  limpo"
        print(f"{'build.py (capítulos e partes)':32} "
              + " ".join(f"{por[c] or '.':>10}" for c in CODIGOS) + marca)
        if inventario and achados_b:
            for ln, cod, txt in achados_b:
                print(f"      build.py:{ln:<5} {cod:<19} {txt}")
    print(f"{'TOTAL':32} " + " ".join(f"{total.get(c, 0):>10}" for c in CODIGOS))
    print()
    for c, desc in CODIGOS.items():
        print(f"  {c:<20} {desc}")
    soma = sum(total.values())
    print(f"\n  {soma} achados. Título de capítulo e de parte vem do build.py; o `#` dos arquivos é seção e entra.")
    ...
    if not any(os.path.isfile(os.path.join(MANUAL, n)) for n in CAPITULOS):
        # A checagem lê a lista CAPITULOS, que é de arquivo do Manual da Guilda, e a
        # régua dela mora na REGRA-DE-VOZ.md de lá. No Bestiário nenhum dos dois
        # existe, e antes da passada 3 isso fazia o --estrito sair 1 sempre.
        print("\n  ROTULO-LONGO não se aplica: nenhum capítulo do Manual da Guilda nesta pasta.")
        rotulo_estourou, entrada_estourou, longos = False, False, None
    elif corte is None:
