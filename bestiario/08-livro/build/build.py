...
MANUAL_DIR = os.path.join(ROOT, "capitulos")
OUT_HTML = os.path.join(BASE, "bestiario.html")   # intermediario, sobrescrito a cada build
...
OUT_PDF = os.path.join(ROOT, "Projeto-M-Bestiario-duas-colunas.pdf"
                       if VARIANTE == "duas" else "Projeto-M-Bestiario.pdf")
CSS_VARIANTE = os.path.join(BASE, "duas-colunas.css") if VARIANTE == "duas" else None
CSS_MAIN = os.path.join(BASE, "bestiario.css")
CSS_MARCAS = os.path.join(BASE, "marcas.css")

# (arquivo, título, kanji, parte) — a ordem aqui é a ordem do livro
CHAPTERS = [
    ("10-o-que-e-uma-maldicao.md",   "Maldições",               "呪", "Catálogo"),
    ("20-os-sete-tipos.md",          "Tipos de criatura",       "類", None),
    ("30-gente-que-vira-maldicao.md","Pessoas e maldições",     "転", None),
    ("40-os-lugares.md",             "Lugares amaldiçoados",    "場", None),

    ("50-o-bloco.md",                "Bloco de inimigo",        "牌", "Montagem"),
    ("60-a-montagem.md",             "Criação de inimigos",     "組", None),
    ("70-area-e-frequencia.md",      "Área e frequência",       "範", None),

    ("80-seis-maldicoes.md",         "Maldições prontas",       "鬼", "Fichas prontas"),
    ("90-referencia.md",             "Referência rápida",       "早", None),
]

# Frente do livro: não recebem número de capítulo.
FRONT = [
    ("05-abertura.md",    "Introdução",               "序"),
    ("07-vocabulario.md", "Vocabulário",              "語"),
]
...
NOTA_CAPA = (
    "O que as maldições são, de onde elas vêm, e como montar uma "
    "para a mesa de hoje à noite."
)
...
def cola_sabor(soup):
    """O paragrafo que ABRE uma secao nao pode ser a ultima coisa da coluna.

    O `break-after: avoid` do titulo amarra ele ao elemento seguinte e PARA AI. O
    padrao que sobrava era titulo + primeiro paragrafo no pe da pagina, com o
    conteudo da secao na seguinte — o leitor virava a pagina com o nome da coisa e
    uma frase, e nada para usar.

    **Nasceu na v0.208 valendo so para a linha de sabor em italico** — as Origens e
    os Caminhos, 42 no livro —, e a v0.210 abriu para TODO primeiro paragrafo, que
    sao 280. *Medido no PDF de duas colunas, construindo as tres formas:*

        forma                          paginas  so o titulo  ate 2 linhas  ate 4
        so italico (v0.208)                144            2            16      29
        paragrafo curto ou italico         146            0            11      26
        todo primeiro paragrafo            147            2            10      20

    **A terceira foi a escolhida: tres paginas em 144 e a melhor nas duas colunas
    ...
    que vier` fecha sozinha e serve para caixa, prosa, tabela e titulo do mesmo
    jeito.

    *`break-before: avoid` em toda caixa ja foi medido e reprovou — a v0.146 achou
    nove paginas a mais —, e e por isso que o alvo e o par e nao a caixa.*
    """
    n = 0
